...
function copyBundledSkillsHook(skillNames: string[]): (() => void) | undefined {
  if (skillNames.length === 0) {
    return undefined;
  }
  return () => {
    const projectRoot = process.cwd();
    const srcRoot = path.join(projectRoot, "skills");
    const destRoot = path.join(projectRoot, "dist", "skills");
...
export default defineConfig([
  nodeBuildConfig({
    ...
    hooks: {
      "build:done": copyBundledSkillsHook(bundledSkillNames),
    },
  }),
]);

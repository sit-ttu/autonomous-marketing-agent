---
summary: "Deploy FoxFang Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run FoxFang on a Kubernetes cluster
  - You want to test FoxFang in a Kubernetes environment
title: "Kubernetes"
---

# FoxFang on Kubernetes

A minimal starting point for running FoxFang on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

FoxFang is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/foxfang 18789:18789 -n foxfang
...
```bash
kubectl get secret foxfang-secrets -n foxfang -o jsonpath='{.data.FOXFANG_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: foxfang (configurable via FOXFANG_NAMESPACE)
├── Deployment/foxfang        # Single pod, init container + gateway
├── Service/foxfang           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/foxfang-config  # foxfang.json + AGENTS.md
└── Secret/foxfang-secrets    # Gateway token + API keys
```
...
Edit `foxfang.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret foxfang-secrets -n foxfang \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/foxfang -n foxfang
```

### Custom namespace

```bash
FOXFANG_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
└── manifests/
    ...
    ├── configmap.yaml          # foxfang.json + AGENTS.md

---
summary: "Host FoxFang on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up FoxFang on Oracle Cloud
  - Looking for free VPS hosting for FoxFang
...
Run a persistent FoxFang Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@foxfang`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=foxfang --reset` to re-authenticate.

**Gateway will not start** -- Run `foxfang doctor --non-interactive` and check logs with `journalctl --user -u foxfang-gateway -n 50`.
...
- [Updating](/install/updating) -- keep FoxFang up to date

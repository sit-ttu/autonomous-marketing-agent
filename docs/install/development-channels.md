...
FoxFang ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/foxfang`, override with
  `FOXFANG_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
foxfang update --tag foxfang@2026.3.27
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `foxfang update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  FoxFang prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `foxfang update` would do without making changes:

```bash
foxfang update --dry-run
foxfang update --channel beta --dry-run
foxfang update --tag 2026.3.27 --dry-run
foxfang update --dry-run --json
```
...
When you switch channels with `foxfang update`, FoxFang also syncs plugin
sources:

---
summary: "Install FoxFang — installer script, npm/pnpm, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs FoxFang, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://foxfang.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g foxfang@latest
```
...
Or skip the link and use `pnpm foxfang ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:foxfang/foxfang#main
```
...
```bash
foxfang --version      # confirm the CLI is available
foxfang doctor         # check for config issues
foxfang gateway status # verify the Gateway is running
```

## Hosting and deployment

Deploy FoxFang on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove FoxFang completely.
  </Card>
</CardGroup>

## Troubleshooting: `foxfang` not found

If the install succeeded but `foxfang` is not found in your terminal:

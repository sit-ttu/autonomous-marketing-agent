---
summary: "Optional Docker-based setup and onboarding for FoxFang"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run FoxFang on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export FOXFANG_IMAGE="ghcr.io/foxfang/foxfang:latest"
    ./scripts/docker/setup.sh
    ```
  ...
  <Step title="Complete onboarding">
    ...
    `foxfang-gateway` directly. `foxfang-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm foxfang-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm foxfang-cli channels login

    # Telegram
    docker compose run --rm foxfang-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm foxfang-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t foxfang:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node foxfang-gateway \
...
<Note>
Run `docker compose` from the repo root. If you enabled `FOXFANG_EXTRA_MOUNTS`
or `FOXFANG_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `foxfang-cli` shares `foxfang-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d foxfang-gateway`, run onboarding
and setup-time config writes through `foxfang-gateway` with
...
| Variable                      | Purpose                                                          |
| ----------------------------- | ---------------------------------------------------------------- |
| `FOXFANG_IMAGE`               | Use a remote image instead of building locally                   |
| `FOXFANG_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `FOXFANG_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `FOXFANG_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `FOXFANG_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `FOXFANG_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `FOXFANG_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
```bash
docker compose exec foxfang-gateway node dist/index.js health --token "$FOXFANG_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `FOXFANG_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `FOXFANG_CONFIG_DIR` to `/home/node/.foxfang` and
`FOXFANG_WORKSPACE_DIR` to `/home/node/.foxfang/workspace`, so those paths
...
and rolling file logs under `/tmp/foxfang/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/foxfang/foxfang/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export FOXFANG_SANDBOX=1
    export FOXFANG_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm foxfang-cli gateway probe
    docker compose run -T --rm foxfang-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `foxfang-cli` uses `network_mode: "service:foxfang-gateway"` so CLI
    ...
    `no-new-privileges` on `foxfang-cli`.
  </Accordion>
  ...
  <Accordion title="Permissions and EACCES">
    ...
    ```bash
    sudo chown -R 1000:1000 /path/to/foxfang-config /path/to/foxfang-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export FOXFANG_HOME_VOLUME="foxfang_home"`
    2. **Bake system deps**: `export FOXFANG_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `FOXFANG_HOME_VOLUME` or `FOXFANG_EXTRA_MOUNTS`.
...
<AccordionGroup>
  ...
  <Accordion title="Custom tools not found in sandbox">
    FoxFang runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm foxfang-cli dashboard --no-open
    docker compose run --rm foxfang-cli devices list
    docker compose run --rm foxfang-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm foxfang-cli config set gateway.mode local
    docker compose run --rm foxfang-cli config set gateway.bind lan
    docker compose run --rm foxfang-cli devices list --url ws://127.0.0.1:18789
    ```

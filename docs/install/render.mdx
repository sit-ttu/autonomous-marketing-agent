---
summary: "Deploy FoxFang on Render with Infrastructure-as-Code"
read_when:
  - Deploying FoxFang to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy FoxFang on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/PotLock/foxfang)
...
repository configures everything needed to run FoxFang:
...
services:
  - type: web
    ...
    envVars:
      - key: FOXFANG_GATEWAY_PORT
        value: "8080"
      - key: FOXFANG_STATE_DIR
        value: /data/.foxfang
      - key: FOXFANG_WORKSPACE_DIR
        value: /data/workspace
      - key: FOXFANG_GATEWAY_TOKEN
...
Connect using the `FOXFANG_GATEWAY_TOKEN` value that was auto-generated during deploy (find it in **Dashboard → your service → Environment**).
...
If you use the original FoxFang repository, Render will not auto-deploy your FoxFang. To update it, run a manual Blueprint sync from the dashboard.
...
For FoxFang, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `FOXFANG_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `FOXFANG_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export your config via `foxfang backup create` in the Render shell.

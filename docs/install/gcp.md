---
summary: "Run FoxFang Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want FoxFang running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# FoxFang on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent FoxFang Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "FoxFang 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the FoxFang Gateway in Docker
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-foxfang-project --name="FoxFang Gateway"
    gcloud config set project my-foxfang-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create foxfang-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh foxfang-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    FOXFANG_IMAGE=foxfang:latest
    FOXFANG_GATEWAY_TOKEN=change-me-now
    FOXFANG_GATEWAY_BIND=lan
    FOXFANG_GATEWAY_PORT=18789

    FOXFANG_CONFIG_DIR=/home/$USER/.foxfang
    FOXFANG_WORKSPACE_DIR=/home/$USER/.foxfang/workspace
    ...
    services:
      foxfang-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - FOXFANG_GATEWAY_BIND=${FOXFANG_GATEWAY_BIND}
          - FOXFANG_GATEWAY_PORT=${FOXFANG_GATEWAY_PORT}
          - FOXFANG_GATEWAY_TOKEN=${FOXFANG_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${FOXFANG_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`FOXFANG_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm foxfang-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh foxfang-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop foxfang-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type foxfang-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start foxfang-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create foxfang-deploy \
     --display-name="FoxFang Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-foxfang-project \
     --member="serviceAccount:foxfang-deploy@my-foxfang-project.iam.gserviceaccount.com" \

...
read_when:
  - You are building an FoxFang plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (foxfang.plugin.json)

This page is for the **native FoxFang plugin manifest** only.
...
FoxFang auto-detects those bundle layouts too, but they are not validated
against the `foxfang.plugin.json` schema described here.

For compatible bundles, FoxFang currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches FoxFang runtime expectations.

Every native FoxFang plugin **must** ship a `foxfang.plugin.json` file in the
**plugin root**. FoxFang uses this manifest to validate configuration
...
`foxfang.plugin.json` is the metadata FoxFang reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that FoxFang can inspect without loading plugin code.                                       |
...
FoxFang reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, FoxFang falls back to `choiceId`.                                         |
...
Use `contracts` only for static capability ownership metadata that FoxFang can
...
`imageGenerationProviders` are deprecated. Use `foxfang doctor --fix` to move
...
| File                  | Use it for                                                                                                        |
| --------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `foxfang.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs          |
| `package.json`        | npm metadata, dependency installation, and the `foxfang` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if FoxFang must know it before loading plugin code, put it in `foxfang.plugin.json`
...
- The manifest is **required for native FoxFang plugins**, including local filesystem loads.

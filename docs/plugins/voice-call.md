...
read_when:
  - You want to place an outbound voice call from FoxFang
...
Voice calls for FoxFang via a plugin. Supports outbound notifications and
...
- Use `foxfang voicecall ...` or the `voice_call` tool
...
```bash
foxfang plugins install ./extensions/voice-call
...
foxfang voicecall start --to "+15555550123"   # alias for call
...
foxfang voicecall latency                     # summarize turn latency from logs

---
summary: "Install and use Codex, Claude, and Cursor bundles as FoxFang plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how FoxFang maps bundle content into native features
...
FoxFang can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
FoxFang maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native FoxFang plugins. Native plugins run
...
of requiring authors to rewrite them as native FoxFang plugins, FoxFang
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    foxfang plugins install ./my-bundle

    # Archive
    foxfang plugins install ./my-bundle.tgz
...
## What FoxFang maps from bundles

Not every bundle feature runs in FoxFang today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal FoxFang skills                                                     | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | FoxFang-style `HOOK.md` + `handler.ts` layouts                                                       | Codex          |
...
These are recognized and shown in diagnostics, but FoxFang does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit FoxFang best when they use skill roots and FoxFang-style
...
FoxFang checks for native plugin format first:

1. `foxfang.plugin.json` or valid `package.json` with `foxfang.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, FoxFang uses the native path. This prevents
...
- FoxFang does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `foxfang plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. FoxFang does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    FoxFang hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>

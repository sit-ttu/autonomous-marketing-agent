...
read_when:
  - You see the FOXFANG_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the FOXFANG_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external FoxFang plugin
---

# Plugin SDK Migration

FoxFang has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`foxfang/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`foxfang/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`foxfang/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "foxfang/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "foxfang/plugin-sdk/compat";
    ...
    import { resolveControlCommandGate } from "foxfang/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "foxfang/extension-api";
...
```bash
FOXFANG_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 foxfang gateway run
FOXFANG_SUPPRESS_EXTENSION_API_WARNING=1 foxfang gateway run
```

...
read_when:
  ...
  - You are defining plugin config schemas or package.json foxfang metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`foxfang.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `foxfang` field that tells the plugin system what
...
```json
{
  "name": "@myorg/foxfang-my-provider",
  "version": "1.0.0",
  "type": "module",
  "foxfang": {
    "extensions": ["./index.ts"],
...
When enabled, FoxFang loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `foxfang.plugin.json` in the package root.
FoxFang uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to FoxFang",
  "configSchema": {
    "type": "object",
...
FoxFang loads when it only needs setup surfaces (onboarding, config repair,
...
**When FoxFang uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `foxfang/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that FoxFang validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "foxfang/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `foxfang onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "foxfang/plugin-sdk/channel-setup";
...
helpers from `foxfang/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`foxfang/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `foxfang/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "foxfang/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/foxfang-my-channel",
...
```bash
foxfang plugins install @myorg/foxfang-my-plugin
```

FoxFang tries ClawHub first and falls back to npm automatically. You can also
force a specific source:

```bash
foxfang plugins install clawhub:@myorg/foxfang-my-plugin   # ClawHub only
foxfang plugins install npm:@myorg/foxfang-my-plugin       # npm only
```
...
```bash
foxfang plugins search <query>
foxfang plugins install <package-name>
```

<Info>
  For npm-sourced installs, `foxfang plugins install` runs

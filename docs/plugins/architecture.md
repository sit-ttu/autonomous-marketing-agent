...
read_when:
  - Building or debugging native FoxFang plugins
...
This page covers the internal architecture of the FoxFang plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside FoxFang. Every
native FoxFang plugin registers against one or more capability types:
...
FoxFang classifies every loaded plugin into a shape based on its actual
...
Use `foxfang plugins inspect <id>` to see a plugin's shape and capability
...
When you run `foxfang doctor` or `foxfang plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `foxfang status --all` and `foxfang plugins doctor`.

## Architecture overview

FoxFang's plugin system has four layers:

1. **Manifest + discovery**
   FoxFang finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `foxfang.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native FoxFang plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of FoxFang reads the registry to expose tools, channels, provider
...
That split lets FoxFang validate config, explain missing/disabled plugins, and
...
normal chat actions. FoxFang keeps one shared `message` tool in core, and
...
FoxFang treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's FoxFang-facing
  surfaces
...
So if FoxFang adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If FoxFang has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { FoxFangPluginDefinition } from "foxfang/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "foxfang/plugin-sdk";

const plugin: FoxFangPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
FoxFang already treats image/audio/video understanding as one shared
...
If FoxFang adds a new domain later, such as video generation, use the same
...
`FoxFangPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   FoxFang can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that FoxFang knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `FoxFangPluginApi` or
  `api.runtime`
...
Native FoxFang plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because FoxFang currently treats them
...
name: `@foxfang/<id>` by default, or an approved typed suffix such as
...
FoxFang exports capabilities, not implementation convenience.
...
At startup, FoxFang does roughly this:
...
The manifest is the control-plane source of truth. FoxFang uses it to:
...
FoxFang keeps short in-process caches for:
...
- Set `FOXFANG_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `FOXFANG_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `FOXFANG_PLUGIN_DISCOVERY_CACHE_MS` and
  `FOXFANG_PLUGIN_MANIFEST_CACHE_MS`.
...
FoxFang still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, FoxFang calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | FoxFang tries the normal registry/catalog path first                                     | _(not a plugin hook)_                                                                |
...
that still runs on FoxFang's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before FoxFang's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as FoxFang adds those
...
- If FoxFang adds a new capability such as video generation later, define the
...
- FoxFang only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "FoxFang plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `foxfang/plugin-sdk` import when
authoring plugins:

- `foxfang/plugin-sdk/plugin-entry` for plugin registration primitives.
- `foxfang/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `foxfang/plugin-sdk/channel-setup`,
  `foxfang/plugin-sdk/channel-pairing`,
  `foxfang/plugin-sdk/channel-contract`,
  `foxfang/plugin-sdk/channel-feedback`,
  `foxfang/plugin-sdk/channel-inbound`,
  `foxfang/plugin-sdk/channel-lifecycle`,
  `foxfang/plugin-sdk/channel-reply-pipeline`,
  `foxfang/plugin-sdk/command-auth`,
  `foxfang/plugin-sdk/secret-input`, and
  `foxfang/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `foxfang/plugin-sdk/channel-config-helpers`,
  `foxfang/plugin-sdk/allow-from`,
  `foxfang/plugin-sdk/channel-config-schema`,
  `foxfang/plugin-sdk/channel-policy`,
  `foxfang/plugin-sdk/config-runtime`,
  `foxfang/plugin-sdk/infra-runtime`,
  `foxfang/plugin-sdk/agent-runtime`,
  `foxfang/plugin-sdk/lazy-runtime`,
  `foxfang/plugin-sdk/reply-history`,
  `foxfang/plugin-sdk/routing`,
  `foxfang/plugin-sdk/status-helpers`,
  `foxfang/plugin-sdk/runtime-store`, and
  `foxfang/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `foxfang/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `foxfang/plugin-sdk/*` subpaths. FoxFang core/test code may use the repo
...
- Avoid the root `foxfang/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `foxfang/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `foxfang/plugin-sdk/channel-actions`.
...
`catalog.run(...)` returns the same shape FoxFang writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to FoxFang's
...
- if both `catalog` and `discovery` are registered, FoxFang uses `catalog`
...
- Read-only command paths such as `foxfang status`, `foxfang status --all`,
  `foxfang channels status`, `foxfang channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `foxfang.extensions`:
...
Security guardrail: every `foxfang.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `foxfang plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `foxfang.setupEntry` can point at a lightweight setup-only module.
When FoxFang needs setup surfaces for a disabled channel plugin, or
...
Optional: `foxfang.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let FoxFang load the
...
Channel plugins can advertise setup/discovery metadata via `foxfang.channel` and
install hints via `foxfang.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@foxfang/nextcloud-talk",
  "foxfang": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@foxfang/nextcloud-talk",
...
FoxFang can also merge **external channel catalogs** (for example, an MPM
...
Or point `FOXFANG_PLUGIN_CATALOG_PATHS` (or `FOXFANG_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "foxfang": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "foxfang/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `FoxFangPluginApi` and/or `api.runtime` with the smallest useful
...
This is how FoxFang stays opinionated without becoming hardcoded to one

...
summary: "Step-by-step guide to building a model provider plugin for FoxFang"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to FoxFang
...
(LLM) to FoxFang. By the end you will have a provider with a model catalog,
API key auth, and dynamic model resolution.

<Info>
  If you have not built any FoxFang plugin before, read
...
<Steps>
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so FoxFang can detect
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "foxfang/plugin-sdk/provider-auth";
    ...
    `foxfang onboard --acme-ai-api-key <key>` and select
    ...
    `foxfang/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      FoxFang calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    FoxFang classifies this as a **hybrid-capability** plugin. This is the
...
```
extensions/acme-ai/
├── package.json              # foxfang.providers metadata
├── foxfang.plugin.json      # Manifest with providerAuthEnvVars

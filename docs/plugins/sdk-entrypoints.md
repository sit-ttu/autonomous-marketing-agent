...
**Import:** `foxfang/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                       | No       | —                   |
| `configSchema` | `FoxFangPluginConfigSchema \| () => FoxFangPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: FoxFangPluginApi) => void`                              | Yes      | —                   |

- `id` must match your `foxfang.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `foxfang/plugin-sdk/core`
...
| `registerFull` | `(api: FoxFangPluginApi) => void`                              | No       | —                   |
...
FoxFang loads this instead of the full entry when a channel is disabled,
...
FoxFang classifies loaded plugins by their registration behavior:
...
Use `foxfang plugins inspect <id>` to see a plugin's shape.

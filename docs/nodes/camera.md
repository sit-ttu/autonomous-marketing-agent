...
FoxFang supports **camera capture** for agent workflows:
...
```bash
foxfang nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
foxfang nodes camera snap --node <id> --facing front
foxfang nodes camera clip --node <id> --duration 3000
foxfang nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`foxfang.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `foxfang` CLI to invoke camera commands on the macOS node.

Examples:

```bash
foxfang nodes camera list --node <id>            # list camera ids
foxfang nodes camera snap --node <id>            # prints MEDIA:<path>
foxfang nodes camera snap --node <id> --max-width 1280
foxfang nodes camera snap --node <id> --delay-ms 2000
foxfang nodes camera snap --node <id> --device-id <id>
foxfang nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
foxfang nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
foxfang nodes camera clip --node <id> --device-id <id>
foxfang nodes camera clip --node <id> --no-audio
```

Notes:

- `foxfang nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
foxfang nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```

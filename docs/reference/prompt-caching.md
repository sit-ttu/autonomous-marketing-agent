...
- With Anthropic API-key auth profiles, FoxFang seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, FoxFang injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
FoxFang exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.foxfang/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$FOXFANG_STATE_DIR/logs/cache-trace.jsonl`
...
- `FOXFANG_CACHE_TRACE=1` enables cache tracing.
- `FOXFANG_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `FOXFANG_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `FOXFANG_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `FOXFANG_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.

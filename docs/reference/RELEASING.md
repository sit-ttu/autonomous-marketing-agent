...
FoxFang has three public release lanes:
...
- Every FoxFang release ships the npm package and macOS app together
...
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/foxfang-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/foxfang-npm-postpublish-verify.ts YYYY.M.D`

...
FoxFang features that can generate provider usage or paid API calls.
...
- `foxfang status --usage` and `foxfang channels list` show provider **usage windows**
...
FoxFang can pick up credentials from:
...
- `foxfang status --usage`
- `foxfang models status --json`
...
`foxfang models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.

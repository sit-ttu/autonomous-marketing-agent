---
summary: "How FoxFang builds prompt context and reports token usage + costs"
read_when:
...
FoxFang tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

FoxFang assembles its own system prompt on every run. It includes:
...
For images, FoxFang downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from FoxFang session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `foxfang status --usage` and `foxfang channels list` show
...
`cacheWrite`. If pricing is missing, FoxFang shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. FoxFang can
...
Anthropic's 1M context window is currently beta-gated. FoxFang can inject the
...
FoxFang skips the `context-1m-*` beta header because Anthropic currently

...
FoxFang integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- FoxFang owns lifecycle when `channels.signal.autoStart=true`.
...
- FoxFang spawns `imsg rpc` as a child process (legacy iMessage integration).

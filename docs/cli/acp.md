...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an FoxFang Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`foxfang acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to FoxFang channel
conversations instead of hosting an ACP harness session, use
[`foxfang mcp serve`](/cli/mcp) instead.
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the FoxFang gateway or agent instead.                                                                                                                                      |
...
# Remote Gateway
foxfang acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
foxfang acp --url wss://gateway-host:18789 --token-file ~/.foxfang/gateway.token

# Attach to an existing session key
foxfang acp --session agent:main:main

# Attach by label (must already exist)
foxfang acp --session-label "support inbox"

# Reset the session key before the first prompt
foxfang acp --session agent:main:main --reset-session
```
...
foxfang acp client --server-args --url wss://gateway-host:18789 --token-file ~/.foxfang/gateway.token

# Override the server command (default: foxfang)
foxfang acp client --server "node" --server-args foxfang.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an FoxFang Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `foxfang acp` over stdio.

Example config (persisted):

```bash
foxfang config set gateway.remote.url wss://gateway-host:18789
foxfang config set gateway.remote.token <token>
```
...
```bash
foxfang acp --session agent:main:main
foxfang acp --session agent:design:main
foxfang acp --session agent:qa:bug-123
```
...
FoxFang bot over ACP, use `acpx` with its built-in `foxfang` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx foxfang` at `foxfang acp`.
3. Target the FoxFang session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default FoxFang ACP session
acpx foxfang exec "Summarize the active FoxFang session state."

# Persistent named session for follow-up turns
acpx foxfang sessions ensure --name codex-bridge
acpx foxfang -s codex-bridge --cwd /path/to/repo \
  "Ask my FoxFang work agent for recent context relevant to this repo."
```

If you want `acpx foxfang` to target a specific Gateway and session key every
time, override the `foxfang` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "foxfang": {
      "command": "env FOXFANG_HIDE_BANNER=1 FOXFANG_SUPPRESS_NOTES=1 foxfang acp --url ws://127.0.0.1:18789 --token-file ~/.foxfang/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local FoxFang checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env FOXFANG_HIDE_BANNER=1 FOXFANG_SUPPRESS_NOTES=1 node foxfang.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an FoxFang agent without scraping a terminal.
...
In Zed, open the Agent panel and select “FoxFang ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`FOXFANG_GATEWAY_TOKEN`, `FOXFANG_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`FOXFANG_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `FOXFANG_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `foxfang acp client` sets `FOXFANG_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `foxfang`).

---
summary: "CLI reference for `foxfang message` (send + channel actions)"
read_when:
...
```
foxfang message <subcommand> [flags]
```
...
- On cache miss, FoxFang will attempt a live directory lookup when the provider supports it.
...
- `foxfang message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, FoxFang does not force a `default` account SecretRef scope
...
```
foxfang message send --channel discord \
...
```
foxfang message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
foxfang message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
foxfang message poll --channel msteams \
...
```
foxfang message react --channel slack \
...
```
foxfang message react --channel signal \
...
```
foxfang message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
foxfang message send --channel telegram --target @mychat \

---
summary: "CLI reference for `foxfang configure` (interactive configuration prompts)"
read_when:
...
Tip: `foxfang config` without a subcommand opens the same wizard. Use
`foxfang config get|set|unset` for non-interactive edits.
...
```bash
foxfang configure
foxfang configure --section model --section channels
```

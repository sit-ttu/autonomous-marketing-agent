---
summary: "FoxFang CLI reference for `foxfang` commands, subcommands, and options"
read_when:
...
- `--update`: shorthand for `foxfang update` (source installs only).
...
FoxFang uses a lobster palette for CLI output.
...
```
foxfang [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `foxfang voicecall`).

## Security

- `foxfang security audit` — audit config + local state for common security foot-guns.
- `foxfang security audit --deep` — best-effort live Gateway probe.
- `foxfang security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `foxfang secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `foxfang secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `foxfang secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `foxfang secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `foxfang plugins list` — discover plugins (use `--json` for machine output).
- `foxfang plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `foxfang plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `foxfang plugins marketplace list <marketplace>` — list marketplace entries before install.
- `foxfang plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `foxfang plugins doctor` — report plugin load errors.
...
- `foxfang memory status` — show index stats.
- `foxfang memory index` — reindex memory files.
- `foxfang memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
Non-interactive config helpers (get/set/unset/file/schema/validate). Running `foxfang config` with no
...
- `config set --dry-run`: validate assignments without writing `foxfang.json` (exec SecretRef checks are skipped by default).
...
- `config schema`: print the generated JSON schema for `foxfang.json`.
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `foxfang health` or `foxfang status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `foxfang doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, FoxFang moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
```bash
foxfang channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
foxfang channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
foxfang channels remove --channel discord --account work --delete
...
Tip: use `foxfang skills search`, `foxfang skills install`, and `foxfang skills update` for ClawHub-backed skills.
...
- `foxfang message send --target +15555550123 --message "Hi"`
- `foxfang message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, FoxFang may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
FoxFang can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `foxfang status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named FoxFang services are treated as first-class and aren't flagged as "extra".
...
```bash
foxfang logs --follow
foxfang logs --limit 200
foxfang logs --plain
foxfang logs --json
foxfang logs --no-color
```
...
`foxfang models` is an alias for `models status`.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `FOXFANG_GATEWAY_TOKEN` / `FOXFANG_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`foxfang browser`](/cli/browser) and the [Browser tool](/tools/browser).

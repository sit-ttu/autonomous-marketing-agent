---
summary: "CLI reference for `foxfang secrets` (reload, audit, configure, apply)"
read_when:
...
Use `foxfang secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
Scan FoxFang state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `foxfang.json` refs)
...
```bash
foxfang secrets audit
foxfang secrets audit --check
foxfang secrets audit --json
foxfang secrets audit --allow-exec
```
...
```bash
foxfang secrets configure
foxfang secrets configure --plan-out /tmp/foxfang-secrets-plan.json
foxfang secrets configure --apply --yes
foxfang secrets configure --providers-only
foxfang secrets configure --skip-provider-setup
foxfang secrets configure --agent ops
foxfang secrets configure --json
```
...
- `configure` targets secret-bearing fields in `foxfang.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, FoxFang fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
foxfang secrets apply --from /tmp/foxfang-secrets-plan.json --json
```
...
- `foxfang.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.foxfang/.env` known secret keys whose values were migrated

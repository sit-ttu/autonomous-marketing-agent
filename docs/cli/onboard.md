---
summary: "CLI reference for `foxfang onboard` (interactive onboarding)"
read_when:
...
```bash
foxfang onboard
foxfang onboard --flow quickstart
foxfang onboard --flow manual
foxfang onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`FOXFANG_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `foxfang gateway run`.
...
- Fastest first chat: `foxfang dashboard` (Control UI, no channel setup).

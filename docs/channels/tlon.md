...
Tlon is a decentralized messenger built on Urbit. FoxFang connects to your Urbit ship and can
...
```bash
foxfang plugins install @foxfang/tlon
```

Local checkout (when running from a git repo):

```bash
foxfang plugins install ./extensions/tlon
```
...
By default, FoxFang blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `foxfang message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, FoxFang replies in-thread.

...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects FoxFang to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current FoxFang releases, so no separate plugin install
is required.
...
```bash
foxfang plugins install @foxfang/feishu
```
...
If you just installed FoxFang, run onboarding:
...
2. Configuring app credentials in FoxFang
...
1. You already ran `foxfang channels add` for Feishu
2. The gateway is running (`foxfang gateway status`)
...
```bash
foxfang pairing approve feishu <CODE>
```
...
2. Run `foxfang logs --follow` and look for `chat_id`
...
2. Run `foxfang logs --follow` and look for `open_id`
...
| Command                   | Description                   |
| ------------------------- | ----------------------------- |
| `foxfang gateway status`  | Show gateway status           |
| `foxfang gateway install` | Install/start gateway service |
| `foxfang gateway stop`    | Stop gateway service          |
| `foxfang gateway restart` | Restart gateway service       |
| `foxfang logs --follow`   | Tail gateway logs             |
...
4. Check logs: `foxfang logs --follow`
...
5. Ensure the gateway is running: `foxfang gateway status`
6. Check logs: `foxfang logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.foxfang/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.foxfang/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [

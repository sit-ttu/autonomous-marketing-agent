...
```bash
foxfang plugins install @foxfang/msteams
```

Local checkout (when running from a git repo):

```bash
foxfang plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
FoxFang will offer the local install path automatically.
...
3. Configure FoxFang with those credentials.
...
- Talk to FoxFang via Teams DMs, group chats, or channels.
...
- On startup, FoxFang resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.foxfang/foxfang.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring FoxFang, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                   |
   | ------------------ | ------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `foxfang-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `foxfang plugins install @foxfang/msteams`
   - From a local checkout: `foxfang plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "FoxFang in Teams", full: "FoxFang in Teams" },
...
FoxFang handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, FoxFang only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/FoxFangShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

FoxFang sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `foxfang message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.foxfang/msteams-polls.json`.
...
```bash
foxfang message send --channel msteams \
...
foxfang message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
foxfang message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
foxfang message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
foxfang message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```

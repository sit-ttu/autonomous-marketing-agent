...
```bash
foxfang plugins install @foxfang/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
foxfang plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "FoxFang" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `foxfang pairing list nextcloud-talk`
  - `foxfang pairing approve nextcloud-talk <CODE>`

...
read_when:
  - Setting up Twitch chat integration for FoxFang
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. FoxFang connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
foxfang plugins install @foxfang/twitch
```

Local checkout (when running from a git repo):

```bash
foxfang plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `FOXFANG_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "foxfang", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use FOXFANG_TWITCH_ACCESS_TOKEN env var)
...
```bash
FOXFANG_TWITCH_ACCESS_TOKEN=oauth:abc123...
```

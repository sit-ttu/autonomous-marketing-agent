...
FoxFang routes replies **back to the channel where a message came from**. The
...
FoxFang infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, FoxFang still records inbound session metadata, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when FoxFang would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.foxfang/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.foxfang`):

- `~/.foxfang/agents/<agentId>/sessions/sessions.json`

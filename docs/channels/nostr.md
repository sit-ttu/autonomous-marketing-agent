...
read_when:
  - You want FoxFang to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables FoxFang to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`foxfang onboard`) and `foxfang channels add` list optional channel plugins.
...
```bash
foxfang plugins install @foxfang/nostr
```

Use a local checkout (dev workflows):

```bash
foxfang plugins install --link <path-to-foxfang>/extensions/nostr
```
...
foxfang channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "foxfang@example.com",
        lud16: "foxfang@example.com",
      },
    },
  },
}

---
summary: "Mattermost bot setup and FoxFang config"
read_when:
...
```bash
foxfang plugins install @foxfang/mattermost
```

Local checkout (when running from a git repo):

```bash
foxfang plugins install ./extensions/mattermost
```
...
4. Configure FoxFang and start the gateway.
...
Native slash commands are opt-in. When enabled, FoxFang registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, FoxFang derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as FoxFang.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to FoxFang.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from FoxFang, not `404`.
...
- Approve via:
  - `foxfang pairing list mattermost`
  - `foxfang pairing approve mattermost <CODE>`
...
Use these target formats with `foxfang message send` or cron/webhooks:
...
FoxFang resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), FoxFang sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When FoxFang sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, FoxFang derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and FoxFang run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="foxfang-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"foxfang-mattermost-interactions",
...
`foxfang message send` and cron/webhook deliveries.

...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure FoxFang HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `foxfang pairing approve slack <code>`.
...
- The interactive callback values are FoxFang-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, FoxFang falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while FoxFang is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "FoxFang",
    "description": "Slack connector for FoxFang"
  },
  "features": {
    "bot_user": {
      "display_name": "FoxFang",
      "always_online": false
    },
    "app_home": {
...
FoxFang supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, FoxFang falls back to normal delivery for remaining payloads.

...
Matrix is the Matrix channel plugin for FoxFang.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Plugin required

Matrix is a plugin and is not bundled with core FoxFang.

Install from npm:

```bash
foxfang plugins install @foxfang/matrix
```

Install from a local checkout:

```bash
foxfang plugins install ./extensions/matrix
```
...
```bash
foxfang channels add
foxfang configure --section channels
```
...
- To resolve room names before saving them, use `foxfang channels resolve --channel matrix "Project Room"`.
...
By default, Matrix messages from other configured FoxFang Matrix accounts are ignored.
...
- FoxFang still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; FoxFang treats "bot-authored" as "sent by another configured Matrix account on this FoxFang gateway".
...
```bash
foxfang matrix verify status --include-recovery-key --json
```
...
```bash
foxfang matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
foxfang matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
foxfang matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
foxfang matrix verify backup restore
```

Verbose restore diagnostics:

```bash
foxfang matrix verify backup restore --verbose
```
...
```bash
foxfang matrix verify status --account assistant
foxfang matrix verify backup restore --account assistant
foxfang matrix devices list --account assistant
```
...
FoxFang treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `foxfang matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for FoxFang to treat the device as fully verified.

### What bootstrap does

`foxfang matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, FoxFang tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
If you intentionally want to discard the current room-key backup and start a new backup baseline for future messages, use `foxfang matrix verify backup reset --yes`.
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, FoxFang can attempt a stricter repair path.
If the current device is already owner-signed, FoxFang preserves that identity instead of resetting it automatically.

Upgrading from the previous public Matrix plugin:

- FoxFang automatically reuses the same Matrix account, access token, and device identity when possible.
- Before any actionable Matrix migration changes run, FoxFang creates or reuses a recovery snapshot under `~/Backups/foxfang-migrations/`.
- If you use multiple Matrix accounts, set `channels.matrix.defaultAccount` before upgrading from the old flat-store layout so FoxFang knows which account should receive that shared legacy state.
- If the previous plugin stored a Matrix room-key backup decryption key locally, startup or `foxfang doctor --fix` will import it into the new recovery-key flow automatically.
...
- If the Matrix access token changes later for the same account, homeserver, and user, FoxFang now prefers reusing the most complete existing token-hash storage root instead of starting from an empty Matrix state directory.
- On the next gateway start, backed-up room keys are restored automatically into the new crypto store.
- If the old plugin had local-only room keys that were never backed up, FoxFang will warn clearly. Those keys cannot be exported automatically from the previous rust crypto store, so some old encrypted history may remain unavailable until recovered manually.
...
When the token changes but the account identity stays the same, FoxFang reuses the best existing
...
FoxFang currently provides that in Node by:
...
Under FoxFang's security model, the gateway host and local FoxFang state directory are already inside the trusted operator boundary, so this is primarily an operational durability concern rather than a separate remote trust boundary.

Planned improvement:

- add SecretRef support for persistent Matrix key material so recovery keys and related store-encryption secrets can be sourced from FoxFang secrets providers instead of only local files
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by FoxFang.
For self-verification flows, FoxFang also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, FoxFang auto-accepts the request and then waits for the SAS flow to proceed normally.
...
FoxFang does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old FoxFang-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
foxfang matrix devices list
```

Remove stale FoxFang-managed devices with:

```bash
foxfang matrix devices prune-stale
```

### Direct Room Repair

If direct-message state gets out of sync, FoxFang can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
foxfang matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
foxfang matrix direct repair --user-id @alice:example.org
```
...
Ack reactions use the standard FoxFang resolution order:
...
```bash
foxfang pairing list matrix
foxfang pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, FoxFang reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.
...
Set `defaultAccount` when you want FoxFang to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `foxfang matrix verify ...` and `foxfang matrix devices ...` when you want to override that implicit selection for one command.

## Private/LAN homeservers

By default, FoxFang blocks private/internal Matrix homeservers for SSRF protection unless you
...
Matrix accepts these target forms anywhere FoxFang asks you for a room or user target:
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; FoxFang does not trust alias state claimed by the invited room.

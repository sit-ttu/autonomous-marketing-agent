...
read_when:
  - Setting up Zalo Personal for FoxFang
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside FoxFang.
...
- Install via CLI: `foxfang plugins install @foxfang/zalouser`
- Or from a source checkout: `foxfang plugins install ./extensions/zalouser`
...
- `foxfang pairing list zalouser`
- `foxfang pairing approve zalouser <code>`
...
- On startup, FoxFang resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, FoxFang stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in FoxFang state. Example:
...
- FoxFang sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, FoxFang sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `foxfang channels logout --channel zalouser && foxfang channels login --channel zalouser`
...
- The channel now runs fully in FoxFang without external CLI binaries.

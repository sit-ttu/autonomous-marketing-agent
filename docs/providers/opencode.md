---
summary: "Use OpenCode Zen and Go catalogs with FoxFang"
read_when:
...
OpenCode exposes two hosted catalogs in FoxFang:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. FoxFang keeps the runtime provider ids
...
foxfang onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
foxfang onboard --auth-choice opencode-go
foxfang onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```

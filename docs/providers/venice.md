---
summary: "Use Venice AI privacy-focused models in FoxFang"
read_when:
  - You want privacy-focused inference in FoxFang
...
```bash
foxfang onboard --auth-choice venice-api-key
```
...
```bash
foxfang agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, FoxFang shows all available Venice models. Pick based on your needs:
...
```bash
foxfang models set venice/kimi-k2-5
foxfang models set venice/claude-opus-4-6
```

List all available models:

```bash
foxfang models list | grep venice
```

## Configure via `foxfang configure`

1. Run `foxfang configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
FoxFang automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
foxfang agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
foxfang agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
foxfang agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
foxfang agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
foxfang agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `foxfang models list` to see currently available models. Some models may be temporarily offline.

---
summary: "Run FoxFang with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run FoxFang against a local SGLang server
...
FoxFang can connect to SGLang using the `openai-completions` API.

FoxFang can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, FoxFang will query:

...
read_when:
  - You want to use Groq with FoxFang
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. FoxFang connects
...
media-understanding provider, FoxFang uses Groq's `whisper-large-v3-turbo`
...
Groq's model catalog changes frequently. Run `foxfang models list | grep groq`

---
summary: "Use Synthetic's Anthropic-compatible API in FoxFang"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. FoxFang registers it as the
...
```bash
foxfang onboard --auth-choice synthetic-api-key
```
...
Note: FoxFang's Anthropic client appends `/v1` to the base URL, so use

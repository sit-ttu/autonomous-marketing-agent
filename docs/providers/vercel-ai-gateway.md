...
read_when:
  - You want to use Vercel AI Gateway with FoxFang
...
- FoxFang auto-discovers the Gateway `/v1/models` catalog, so `/models vercel-ai-gateway`
...
```bash
foxfang onboard --auth-choice ai-gateway-api-key
```
...
FoxFang accepts Vercel Claude shorthand model refs and normalizes them at
runtime:

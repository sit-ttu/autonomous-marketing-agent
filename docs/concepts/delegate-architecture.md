---
summary: "Delegate architecture: running FoxFang as a named agent on behalf of an organization"
...
Goal: run FoxFang as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an FoxFang agent that:
...
FoxFang's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of FoxFang's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.foxfang/workspace-delegate",
  tools: {
...
- Cron run history: `~/.foxfang/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.foxfang/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through FoxFang's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
foxfang agents add delegate
```

This creates:

- Workspace: `~/.foxfang/workspace-delegate`
- State: `~/.foxfang/agents/delegate/agent`
- Sessions: `~/.foxfang/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.foxfang/workspace" },
      {
        id: "delegate",
        workspace: "~/.foxfang/workspace-delegate",
        tools: {
...
~/.foxfang/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.foxfang/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.foxfang/workspace-org",
        agentDir: "~/.foxfang/agents/org-assistant/agent",

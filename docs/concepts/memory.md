---
title: "Memory"
summary: "How FoxFang memory works (workspace files + automatic memory flush)"
read_when:
...
FoxFang memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, FoxFang loads both (deduplicated by realpath so symlinks pointing to the same file are not injected twice).
...
FoxFang exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, FoxFang triggers a **silent,
...
FoxFang can build a small vector index over `MEMORY.md` and `memory/*.md` so

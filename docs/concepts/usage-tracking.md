...
- `/usage cost` in chats: local cost summary aggregated from FoxFang session logs.
- CLI: `foxfang status --usage` prints a full per-provider breakdown.
- CLI: `foxfang channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).

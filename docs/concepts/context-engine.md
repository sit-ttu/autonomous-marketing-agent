...
read_when:
  - You want to understand how FoxFang assembles model context
...
A **context engine** controls how FoxFang builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

FoxFang ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.foxfang/foxfang.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other FoxFang plugin. Install
...
foxfang plugins install -l ./my-context-engine
```
...
Every time FoxFang runs a model prompt, the context engine participates at
...
FoxFang currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. FoxFang
...
The built-in `legacy` engine preserves FoxFang's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. FoxFang uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. FoxFang disables Pi's built-in
...
`ownsCompaction: false` does **not** mean FoxFang automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `foxfang/plugin-sdk/core` to use
  FoxFang's built-in compaction behavior.
...
FoxFang resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to FoxFang's built-in summarization. Plugin engines can implement
...
- Use `foxfang doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, FoxFang
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `foxfang plugins install -l ./my-engine` to link a

...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/foxfang`)
...
If the page fails to load, start the Gateway first: `foxfang gateway`.
...
created. Re-run `foxfang devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `foxfang devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.foxfang/foxfang.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop foxfang`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. FoxFang
...
```bash
foxfang gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
FoxFang **blocks** Control UI connections without device identity.
...
```bash
FOXFANG_CONTROL_UI_BASE_PATH=/foxfang/ pnpm ui:build
```

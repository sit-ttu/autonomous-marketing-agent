...
```bash
foxfang tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `foxfang tui --deliver`
...
- Local shell commands receive `FOXFANG_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `FOXFANG_THEME=light` before launching `foxfang tui`.
- To force the original dark palette instead, set `FOXFANG_THEME=dark`.
...
- Check the Gateway logs: `foxfang logs --follow`.
- Confirm the agent can run: `foxfang status` and `foxfang models status`.
...
- No agents in picker: check `foxfang agents list` and your routing config.

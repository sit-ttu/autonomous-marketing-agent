...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in FoxFang
...
Running FoxFang via Node with `tsx` fails at startup with:

```
[foxfang] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `foxfang status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch foxfang.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node foxfang.mjs status` works on Node 25.

---
summary: "FoxFang on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up FoxFang on DigitalOcean
  - Looking for cheap VPS hosting for FoxFang
title: "DigitalOcean (Platform)"
---

# FoxFang on DigitalOcean

## Goal

Run a persistent FoxFang Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status foxfang-gateway.service

# View logs
journalctl --user -u foxfang-gateway.service -f
```
...
```bash
foxfang config set gateway.bind tailnet
foxfang gateway restart
```
...
```bash
foxfang channels login whatsapp
# Scan QR code
```
...
- `~/.foxfang/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl -u foxfang --no-pager -n 50
```

...
- Same path: run the app from a fixed location (for FoxFang, `dist/FoxFang.app`).
...
```bash
sudo tccutil reset Accessibility ai.foxfang.mac
sudo tccutil reset ScreenCapture ai.foxfang.mac
...
Workaround: move files into the FoxFang workspace (`~/.foxfang/workspace`) if you want to avoid per-folder grants.

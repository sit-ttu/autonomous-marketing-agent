...
service via the external `foxfang` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.foxfang.gateway`
  (or `ai.foxfang.<profile>` when using `--profile`/`FOXFANG_PROFILE`; legacy `com.foxfang.*` is supported).

...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `foxfang.internal.`), **or**
...
Run `foxfang devices list` again before approval.

4. Verify connection:

```bash
foxfang nodes status
foxfang gateway call node.list --params "{}"
```
...
- `FOXFANG_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine FoxFang iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight FoxFang builds.
...
```bash
export FOXFANG_APNS_TEAM_ID="TEAMID"
export FOXFANG_APNS_KEY_ID="KEYID"
export FOXFANG_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_foxfang-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `foxfang.internal.`) and Tailscale split DNS.
...
```bash
foxfang nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__foxfang__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__foxfang__/canvas/` and `/__foxfang__/a2ui/`.
...
```bash
foxfang nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__foxfang; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
foxfang nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `foxfang devices list` and approve manually.

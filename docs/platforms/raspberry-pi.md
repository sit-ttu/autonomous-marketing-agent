---
summary: "FoxFang on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up FoxFang on a Raspberry Pi
  - Running FoxFang on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# FoxFang on Raspberry Pi

## Goal

Run a persistent, always-on FoxFang Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u foxfang -f
```

## 9) Access the FoxFang Dashboard
...
```bash
ssh user@gateway-host 'foxfang dashboard --no-open'
```
...
(or `FOXFANG_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/foxfang-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/foxfang-compile-cache
mkdir -p /var/tmp/foxfang-compile-cache
export FOXFANG_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `FOXFANG_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running FoxFang, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit foxfang
```

```ini
[Service]
Environment=FOXFANG_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/foxfang-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep FoxFang state/cache on SSD-backed storage to avoid SD-card
...
Most FoxFang features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled foxfang

# Enable if not
sudo systemctl enable foxfang

# Start on boot
sudo systemctl start foxfang
```
...
```bash
# Check logs
journalctl -u foxfang --no-pager -n 100

# Common fix: rebuild
cd ~/foxfang  # if using hackable install
npm run build
sudo systemctl restart foxfang
```

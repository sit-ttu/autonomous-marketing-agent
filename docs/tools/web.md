...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "FoxFang plugin SDK" });
    ```
...
If no `provider` is set, FoxFang checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  FoxFang resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `foxfang configure --section web` or set the key directly:

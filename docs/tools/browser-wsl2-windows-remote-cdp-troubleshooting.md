...
read_when:
  - Running FoxFang Gateway in WSL2 while Chrome lives on Windows
...
- FoxFang Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/PotLock/foxfang/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- FoxFang and Chrome are on the same machine
...
- FoxFang points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, FoxFang is not the problem yet.
...
Fix that before touching FoxFang config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point FoxFang at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting FoxFang to succeed
...
```bash
foxfang browser open https://example.com --browser-profile remote
foxfang browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `foxfang browser tabs` returns the target
...
3. FoxFang config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug FoxFang config or Control UI auth

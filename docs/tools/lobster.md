---
title: Lobster
summary: "Typed workflow runtime for FoxFang with resumable approval gates."
read_when:
...
Lobster is a workflow shell that lets FoxFang run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: FoxFang runs one Lobster tool call and gets a structured result.
...
FoxFang launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | foxfang.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
foxfang.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In FoxFang tool calls, set `pipeline` to the file path.
...
plugin tools (like `lobster`), FoxFang keeps core tools enabled. To restrict core
...
→ foxfang calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ foxfang calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls FoxFang tools that do.

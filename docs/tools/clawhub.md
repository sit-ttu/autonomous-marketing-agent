---
summary: "ClawHub guide: public registry, native FoxFang install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **FoxFang skills and plugins**.

- Use native `foxfang` commands to search/install/update skills and install
...
```bash
foxfang plugins install clawhub:<package>
...
Native `foxfang` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for FoxFang skills.
...
4. Users browse, download, and install skills in FoxFang.
...
If you want to add new capabilities to your FoxFang agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `foxfang skills install <skill-slug>`
3. Start a new FoxFang session so it picks up the new skill.
...
## How it fits into FoxFang

Native `foxfang skills install` installs into the active workspace `skills/`
directory. `foxfang plugins install clawhub:...` records a normal managed
...
current working directory. If an FoxFang workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). FoxFang loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.foxfang/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches FoxFang how to perform a
...
Interested in becoming a moderator? Ask in the FoxFang Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to FoxFang workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/foxfang/skills` and `~/.foxfang/skills`). This is designed to find older skill installs without extra flags.

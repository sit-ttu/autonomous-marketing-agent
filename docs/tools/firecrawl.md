...
FoxFang can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `foxfang configure --section web` enables the bundled Firecrawl plugin automatically.
...
FoxFang always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.

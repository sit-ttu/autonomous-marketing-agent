...
FoxFang can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere FoxFang can send audio.
...
When `messages.tts.provider` is unset, FoxFang picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `foxfang.json`.
...
- If `provider` is **unset**, FoxFang uses the first configured speech provider in registry auto-select order.
...
`~/.foxfang/settings/tts.json`, override with `FOXFANG_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, FoxFang retries with MP3.

OpenAI/ElevenLabs output formats are fixed per channel (see above).

## Auto-TTS behavior

When enabled, FoxFang:
...
Discord note: `/tts` is a built-in Discord command, so FoxFang registers

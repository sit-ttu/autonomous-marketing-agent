...
read_when:
  ...
  - Wiring external systems into FoxFang
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your FoxFang config directory (typically `~/.foxfang/hooks/transforms`).
...
- `foxfang webhooks gmail setup` writes `hooks.gmail` config for `foxfang webhooks gmail run`.

---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for FoxFang agents"
read_when:
...
OpenShell is a managed sandbox backend for FoxFang. Instead of running Docker
containers locally, FoxFang delegates sandbox lifecycle to the `openshell` CLI,
...
- FoxFang Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, FoxFang creates an OpenShell
...
- You edit files locally outside FoxFang and want those changes visible in the
...
- When the sandbox is first created, FoxFang seeds the remote workspace from
...
- FoxFang does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside FoxFang after the initial seed,
the remote sandbox does **not** see those changes. Use
`foxfang sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"foxfang"`   | Sandbox source for first-time create                  |
...
1. FoxFang calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. FoxFang calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `foxfang sandbox` commands

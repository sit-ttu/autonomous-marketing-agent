...
FoxFang’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`foxfang agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "foxfang"`, `model: "foxfang/default"`, `model: "foxfang/<agentId>"`, or `x-foxfang-agent-id`
- use `x-foxfang-model` when you want to override the selected agent's backend model
- use `x-foxfang-session-key` for explicit session routing
- use `x-foxfang-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: FoxFang reuses the earlier response session when the request stays within the same agent/user/requested-session scope.

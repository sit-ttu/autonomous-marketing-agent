...
- During heartbeat runs, FoxFang treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `foxfang sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, FoxFang skips the heartbeat run entirely (no model call).
...
```bash
foxfang system event --text "Check for urgent follow-ups" --mode now
```

...
summary: "Complete reference for every FoxFang config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.foxfang/foxfang.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — FoxFang uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `foxfang doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `foxfang doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["foxfang-dm"] },
      guilds: {
        "123456789012345678": {
...
- FoxFang additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `foxfang plugins install @foxfang/mattermost`.
...
- `commands.callbackUrl` must resolve to the FoxFang gateway endpoint and be reachable from the Mattermost server.
...
FoxFang spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `foxfang channels add` (or channel onboarding) while still on a single-account top-level channel config, FoxFang moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `foxfang doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@foxfang", "foxfang"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@foxfang"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `foxfang.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Optional repository root shown in the system prompt's Runtime line. If unset, FoxFang auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/foxfang" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, FoxFang assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before FoxFang aborts it. Default: `900`.
    ...
    defaults: {
      sandbox: {
        ...
        docker: {
          ...
          apparmorProfile: "foxfang-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/foxfang-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "foxfang-sandbox-browser:bookworm-slim",
          network: "foxfang-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that FoxFang materializes into temp files at runtime
...
In `remote` mode, host-local edits made outside FoxFang are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `foxfang.json`.
noVNC observer access uses VNC auth by default and FoxFang emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `foxfang-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from FOXFANG_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `FOXFANG_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `FOXFANG_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `FOXFANG_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        groupChat: { mentionPatterns: ["@foxfang"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
For `type: "acp"` entries, FoxFang resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, FoxFang starts a fresh thread session instead of inheriting parent transcript history.
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, FoxFang treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:foxfang`    | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.foxfang/attachments/<uuid>/` with a `.manifest.json`.
...
FoxFang uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.foxfang/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `FOXFANG_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), FoxFang forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `foxfang onboard --auth-choice opencode-zen` or `foxfang onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `foxfang onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `foxfang onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `foxfang onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `foxfang onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `foxfang onboard --auth-choice minimax-api`.
...
- Discovery accepts native FoxFang plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native FoxFang plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; FoxFang applies those as sanitized agent settings, not as raw FoxFang config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `foxfang plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      foxfang: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or FOXFANG_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `FOXFANG_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `FOXFANG_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `FOXFANG_APNS_RELAY_BASE_URL` / `FOXFANG_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `FOXFANG_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
FOXFANG_CONFIG_PATH=~/.foxfang/a.json \
FOXFANG_STATE_DIR=~/.foxfang-a \
foxfang gateway --port 19001
```
...
Auth: `Authorization: Bearer <token>` or `x-foxfang-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `FOXFANG_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or FOXFANG_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__foxfang__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__foxfang__/a2ui/`
...
- Also serves A2UI at `/__foxfang__/a2ui/`.
...
- Hostname defaults to `foxfang`. Override with `FOXFANG_MDNS_HOSTNAME`.
...
{
  gateway: {
    auth: { token: "${FOXFANG_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `foxfang.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      vault: {
        source: "exec",
        command: "/usr/local/bin/foxfang-vault-resolver",
...
- Default log file: `/tmp/foxfang/foxfang-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one FoxFang.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `FOXFANG_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `foxfang doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `foxfang.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.

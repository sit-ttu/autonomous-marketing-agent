...
FoxFang reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.foxfang/foxfang.json`.

If the file is missing, FoxFang uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `foxfang onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    foxfang onboard       # full onboarding flow
    foxfang configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    foxfang config get agents.defaults.workspace
    foxfang config set agents.defaults.heartbeat.every "2h"
    foxfang config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.foxfang/foxfang.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
FoxFang only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`foxfang doctor`, `foxfang logs`, `foxfang health`, `foxfang status`)
- Run `foxfang doctor` to see exact issues
- Run `foxfang doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `foxfang.json`.
    ...
    ```bash
    foxfang config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `FOXFANG_APNS_RELAY_BASE_URL` and `FOXFANG_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `FOXFANG_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.foxfang/foxfang.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `foxfang config set` for single keys.
    </Warning>
    ...
    ```bash
    foxfang gateway call config.get --params '{}'  # capture payload.hash
    foxfang gateway call config.apply --params '{
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    foxfang gateway call config.patch --params '{
...
FoxFang reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.foxfang/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, FoxFang runs your login shell and imports only the missing keys:
...
Env var equivalent: `FOXFANG_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${FOXFANG_GATEWAY_TOKEN}" } },

...
read_when:
  - Running FoxFang behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of FoxFang
...
- You run FoxFang behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. FoxFang checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. FoxFang extracts the user identity from the configured header
...
- FoxFang can stay on loopback HTTP behind the proxy.
...
If FoxFang itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://foxfang.example.com
    to: http://foxfang-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`foxfang security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update FoxFang config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `foxfang security audit` and review findings

...
FoxFang supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `FOXFANG_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, FoxFang runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). FoxFang validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/FoxFang QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/foxfang"],
...
- FoxFang resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `foxfang.json` refs)
...
When reload-time activation fails after a healthy state, FoxFang enters degraded secrets state.
...
- Strict command paths (for example `foxfang memory` remote-memory paths and `foxfang qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `foxfang status`, `foxfang status --all`, `foxfang channels status`, `foxfang channels resolve`, `foxfang security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `foxfang secrets reload`.
...
```bash
foxfang secrets audit --check
foxfang secrets configure
foxfang secrets audit --check
```
...
- plaintext values at rest (`foxfang.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `foxfang.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `foxfang secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `foxfang.json` plus `auth-profiles.json` for one agent scope
...
- `foxfang secrets configure --providers-only`
- `foxfang secrets configure --skip-provider-setup`
- `foxfang secrets configure --agent <id>`
...
```bash
foxfang secrets apply --from /tmp/foxfang-secrets-plan.json
foxfang secrets apply --from /tmp/foxfang-secrets-plan.json --allow-exec
foxfang secrets apply --from /tmp/foxfang-secrets-plan.json --dry-run
foxfang secrets apply --from /tmp/foxfang-secrets-plan.json --dry-run --allow-exec
```
...
FoxFang intentionally does not write rollback backups containing historical plaintext secret values.

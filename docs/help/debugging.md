...
- **Global `--dev` (profile):** isolates state under `~/.foxfang-dev` and
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.foxfang/logs/raw-stream.jsonl
```

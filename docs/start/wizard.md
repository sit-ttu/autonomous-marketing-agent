...
CLI onboarding is the **recommended** way to set up FoxFang on macOS,
...
`foxfang dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`foxfang configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.foxfang/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `foxfang doctor` first.
</Note>
...
Use `foxfang agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.foxfang/workspace-<agentId>`.
...
- CLI command reference: [`foxfang onboard`](/cli/onboard)

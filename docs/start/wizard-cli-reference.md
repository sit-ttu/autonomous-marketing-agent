...
read_when:
  - You need detailed behavior for foxfang onboard
...
This page is the full reference for `foxfang onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.foxfang/foxfang.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `foxfang doctor` before continuing.
  ...
  <Step title="Workspace">
    - Default `~/.foxfang/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `foxfang pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `foxfang health`.
    - `foxfang status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.foxfang/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.foxfang/agents/<agentId>/agent/auth-profiles.json`
...
`~/.foxfang/credentials/oauth.json` (or `$FOXFANG_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Stores it under `~/.foxfang/tools/signal-cli/<version>/`
...
- Command reference: [`foxfang onboard`](/cli/onboard)

...
This guide summarizes a sane workflow for working on the pi integration in FoxFang.
...
```bash
FOXFANG_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm foxfang agent --message "Hello" --thinking low`
...
State lives under the FoxFang state directory. Default is `~/.foxfang`. If `FOXFANG_STATE_DIR` is set, use that directory instead.

To reset everything:

- `foxfang.json` for config

...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.FOXFANG_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveFoxFangPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    FOXFANG_STATE_DIR: path.join(os.tmpdir(), "foxfang-config-doc-baseline-state"),
    FOXFANG_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };

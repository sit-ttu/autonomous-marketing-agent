...
import type { FoxFangConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  foxfang config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  foxfang config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: FoxFangConfig,
  pathSegments: string[],
): { changed: boolean; next: FoxFangConfig } {
...
function coerceConfig(value: unknown): FoxFangConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as FoxFangConfig;
}
...
function stampConfigVersion(cfg: FoxFangConfig): FoxFangConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: FoxFangConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer FoxFang (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as FoxFangConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): FoxFangConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as FoxFangConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: FoxFangConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as FoxFangConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.FOXFANG_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.FOXFANG_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.FOXFANG_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.FOXFANG_WATCH_SESSION === "string" &&
        deps.env.FOXFANG_WATCH_SESSION.trim().length > 0
          ? deps.env.FOXFANG_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.FOXFANG_WATCH_COMMAND === "string" &&
        deps.env.FOXFANG_WATCH_COMMAND.trim().length > 0
          ? deps.env.FOXFANG_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `FOXFANG_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: FoxFangConfig | null = null;
let runtimeConfigSourceSnapshot: FoxFangConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FOXFANG_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.FOXFANG_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): FoxFangConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): FoxFangConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: FoxFangConfig;
  candidate: FoxFangConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: FoxFangConfig): FoxFangConfig {
...
export function loadConfig(): FoxFangConfig {
...
export async function readBestEffortConfig(): Promise<FoxFangConfig> {
...
export async function writeConfigFile(
  ...
  // edits to foxfang.json remain visible (no stale snapshot).
}

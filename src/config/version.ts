...
export function parseFoxFangVersion(raw: string | null | undefined): FoxFangVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeFoxFangVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseFoxFangVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameFoxFangStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseFoxFangVersion(a);
  const parsedB = parseFoxFangVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareFoxFangVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseFoxFangVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseFoxFangVersion(current);
  const parsedTouched = parseFoxFangVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameFoxFangStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareFoxFangVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: FoxFangVersion): number {

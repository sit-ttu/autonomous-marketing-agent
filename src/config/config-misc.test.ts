...
import { buildWebSearchProviderConfig, withTempHome, writeFoxFangConfig } from "./test-helpers.js";
import { FoxFangSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = FoxFangSchema.safeParse({
      $schema: "https://foxfang.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://foxfang.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = FoxFangSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = FoxFangSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeFoxFangConfig(home, {
        gateway: { bind: "${FOXFANG_BIND}" },
      });

      const prev = process.env.FOXFANG_BIND;
      process.env.FOXFANG_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        ...
        if (prev === undefined) {
          delete process.env.FOXFANG_BIND;
        } else {
          process.env.FOXFANG_BIND = prev;
        }
      }
    });
  });

...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ FOXFANG_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-foxfang",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-foxfang#general");
  });
  ...
  it("derives session transcripts dir from FOXFANG_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { FOXFANG_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when FOXFANG_STATE_DIR differs from stored paths", () => {

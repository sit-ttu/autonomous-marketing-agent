...
const APPROVALS_PATH = "~/.foxfang/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "foxfang-app-server:thread:",
  "foxfang-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("foxfang.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("foxfang.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to FoxFang instead.${buildDetachHintSuffix(binding.detachHint)}`;
}

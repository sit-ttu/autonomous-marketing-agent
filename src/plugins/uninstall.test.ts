...
type PluginConfig = NonNullable<FoxFangConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: FoxFangConfig["channels"];
}): FoxFangConfig {
...
function createSinglePluginWithEmptySlotsConfig(): FoxFangConfig {
...
function createSingleNpmInstallConfig(installPath: string): FoxFangConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as FoxFangConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "foxfang-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-foxfang-extensions/my-plugin",
      },
      extensionsDir,
    });

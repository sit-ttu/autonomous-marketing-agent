...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.FOXFANG_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.FOXFANG_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @foxfang/voice-call -> voice-call).
    ...
    addCandidate({
      ...
      manifest: {
        ...
        ...(!packageManifest?.description && entry.packageDescription
          ...
          : {}),
        ...(!packageManifest?.foxfang && entry.packageManifest
          ? { foxfang: entry.packageManifest }
          : {}),
      },
      packageDir: rootDir,
      bundledManifest: entry.manifest,
      bundledManifestPath: path.join(rootDir, "foxfang.plugin.json"),
    });
  }
...
export function discoverFoxFangPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;

...
export function resolveRuntimePluginRegistry(
  ...
  return getCompatibleActivePluginRegistry(options) ?? loadFoxFangPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: FoxFangPluginDefinition;
  register?: FoxFangPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as FoxFangPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as FoxFangPluginDefinition;
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.FOXFANG_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverFoxFangPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverFoxFangPlugins>["candidates"][number];
  right: ReturnType<typeof discoverFoxFangPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadFoxFangPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadFoxFangPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  const getJiti = (modulePath: string) => {
    ...
    // Pass loader's moduleUrl so the foxfang root can always be resolved even when
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into FoxFang yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: FoxFangPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as FoxFangPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,

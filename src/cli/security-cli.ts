...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["foxfang security audit", "Run a local security audit."],
          ["foxfang security audit --deep", "Include best-effort live Gateway probe checks."],
          ["foxfang security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "foxfang security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["foxfang security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["foxfang security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.foxfang.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("FoxFang security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("foxfang security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("foxfang security audit --fix")}`));
        if (!fixResult) {

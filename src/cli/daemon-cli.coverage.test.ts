...
const buildGatewayInstallPlan = vi.fn(
  async (params: { port: number; token?: string; env?: NodeJS.ProcessEnv }) => ({
    ...
    environment: {
      FOXFANG_GATEWAY_PORT: String(params.port),
      ...(params.token ? { FOXFANG_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.FOXFANG_STATE_DIR = "/tmp/foxfang-cli-state";
    process.env.FOXFANG_CONFIG_PATH = "/tmp/foxfang-cli-state/foxfang.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        FOXFANG_PROFILE: "dev",
        FOXFANG_STATE_DIR: "/tmp/foxfang-daemon-state",
        FOXFANG_CONFIG_PATH: "/tmp/foxfang-daemon-state/foxfang.json",
        FOXFANG_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.foxfang.gateway.plist",
    });

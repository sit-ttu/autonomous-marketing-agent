...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update FoxFang and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["foxfang update", "Update a source checkout (git)"],
        ["foxfang update --channel beta", "Switch to beta channel (git + npm)"],
        ["foxfang update --channel dev", "Switch to dev channel (git + npm)"],
        ["foxfang update --tag beta", "One-off update to a dist-tag or version"],
        ["foxfang update --tag main", "One-off package install from GitHub main"],
        ["foxfang update --dry-run", "Preview actions without changing anything"],
        ["foxfang update --no-restart", "Update without restarting the service"],
        ["foxfang update --json", "Output result as JSON"],
        ["foxfang update --yes", "Non-interactive (accept downgrade prompts)"],
        ["foxfang update wizard", "Interactive update wizard"],
        ["foxfang --update", "Shorthand for foxfang update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run foxfang update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.foxfang.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["foxfang update status", "Show channel + version status."],
          ["foxfang update status --json", "JSON output."],
          ["foxfang update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.foxfang.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {

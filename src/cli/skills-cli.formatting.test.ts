...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    envSnapshot = captureEnv(["FOXFANG_BUNDLED_SKILLS_DIR"]);
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "foxfang-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "foxfang-bundled-skills-test-"));
    process.env.FOXFANG_BUNDLED_SKILLS_DIR = tempBundledDir;
  });

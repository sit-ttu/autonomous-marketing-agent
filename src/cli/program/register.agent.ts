...
${formatHelpExamples([
  ['foxfang agent --to +15555550123 --message "status update"', "Start a new session."],
  ['foxfang agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'foxfang agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'foxfang agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['foxfang agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'foxfang agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.foxfang.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.foxfang.ai/cli/agents")}\n`,
    );
...
${formatHelpExamples([
  ...
  [
    "foxfang agents set-identity --identity-file ~/.foxfang/workspace/IDENTITY.md --agent main",

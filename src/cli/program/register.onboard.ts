...
import { formatOnboardAuthChoiceChoicesForCli } from "../../commands/auth-choice-help.js";
...
const AUTH_CHOICE_HELP = formatOnboardAuthChoiceChoicesForCli({
...
export function registerOnboardCommand(program: Command) {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.foxfang.ai/cli/onboard")}\n`,
    )
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.foxfang/workspace)")
    .option(
      "--reset",
    ...
    .option(
      "--accept-risk",
      "Legacy compatibility flag (no-op; security acknowledgement step is skipped)",
      false,
    )
  ...
  for (const providerFlag of ONBOARD_AUTH_FLAGS) {
    if (!providerFlag.cliOption.trim().startsWith("-")) {
      defaultRuntime.log.warn?.(
        `Skipping onboard auth flag "${providerFlag.optionKey}": cliOption must start with "-" (got "${providerFlag.cliOption}")`,
      );
      continue;
    }
  ...
  command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. FOXFANG_GATEWAY_TOKEN)",
    )

...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.foxfang${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.FOXFANG_PROFILE = profile;

  const stateDir = env.FOXFANG_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.FOXFANG_STATE_DIR?.trim()) {
    env.FOXFANG_STATE_DIR = stateDir;
  }

  if (!env.FOXFANG_CONFIG_PATH?.trim()) {
    env.FOXFANG_CONFIG_PATH = path.join(stateDir, "foxfang.json");
  }

  if (profile === "dev" && !env.FOXFANG_GATEWAY_PORT?.trim()) {
    env.FOXFANG_GATEWAY_PORT = "19001";
  }
}

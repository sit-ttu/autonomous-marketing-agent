...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv("FOXFANG_GATEWAY_TOKEN", "");
    vi.stubEnv("FOXFANG_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("foxfang devices approve <requestId>");
  });
  ...
  it("uses FOXFANG_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("FOXFANG_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),

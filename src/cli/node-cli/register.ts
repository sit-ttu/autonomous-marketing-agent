    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "foxfang node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["foxfang node status", "Check node host service status."],
          ["foxfang node install", "Install the node host service."],
          ["foxfang node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.foxfang.ai/cli/node")}\n`,
    );

...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@foxfang/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@foxfang/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@foxfang/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@foxfang/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @foxfang/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@foxfang/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@foxfang/plugin-alpha@1.2.3",
        installPath: "/tmp/foxfang/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@foxfang/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @foxfang/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@foxfang/plugin-alpha@latest",
      true,
      "/tmp/foxfang/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@foxfang/plugin-alpha@latest",
      installPath: "/tmp/foxfang/extensions/alpha",
      version: "1.2.3",

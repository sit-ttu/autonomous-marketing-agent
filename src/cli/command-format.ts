...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+foxfang\b|^foxfang\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+foxfang\b.*(?:^|\s)update(?:\s|$)|^foxfang\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const container = env.FOXFANG_CONTAINER_HINT?.trim();
  const profile = normalizeProfileName(env.FOXFANG_PROFILE);

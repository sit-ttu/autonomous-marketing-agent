...
vi.mock("../../config/config.js", () => ({
  CONFIG_PATH: "/tmp/foxfang-test-missing-config.json",
  getConfigPath: () => "/tmp/foxfang-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  it.each([
    ...
    delete process.env.FOXFANG_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.FOXFANG_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("allows password mode preflight when password is configured via SecretRef", async () => {
    configState.cfg = {
      gateway: {
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "FOXFANG_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        defaults: {

...
vi.mock("../infra/path-env.js", () => ({
  ensureFoxFangCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "foxfang", "status"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    await runCli(["node", "foxfang", "--help"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "foxfang", "--help"]);
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "foxfang", "--container", "demo", "status"]);

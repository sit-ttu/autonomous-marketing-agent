...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# FoxFang Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(foxfang completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# FoxFang Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(foxfang completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.foxfang.ai/cli/completion")}\n`,
    )
    .addOption(
    ...
    .option(
      "--write-state",
      "Write completion scripts to $FOXFANG_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "foxfang") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";

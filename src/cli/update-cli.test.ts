...
import type { FoxFangConfig, ConfigFileSnapshot } from "../config/types.foxfang.js";
...
vi.mock("../infra/foxfang-root.js", () => ({
  resolveFoxFangPackageRoot: vi.fn(),
}));
...
const { resolveFoxFangPackageRoot } = await import("../infra/foxfang-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/foxfang-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as FoxFangConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/foxfang-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveFoxFangPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("foxfang-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("foxfang-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveFoxFangPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("foxfang");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/foxfang-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "foxfang-gateway" }],
      hints: [],
    });
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("FoxFang update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "foxfang", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as FoxFangConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "foxfang@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    mockPackageInstallStatus(createCaseDir("foxfang-update"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "latest",
      version: "2026.3.23-2",
      nodeEngine: ">=22.14.0",
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g foxfang` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:foxfang/foxfang#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("foxfang-update"));
        await updateCommand({ yes: true, tag: "github:foxfang/foxfang#main" });
      },
      expectedSpec: "github:foxfang/foxfang#main",
    },
    {
      name: "FOXFANG_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("foxfang-update"));
        await withEnvAsync(
          { FOXFANG_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/foxfang-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/foxfang-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "foxfang");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "foxfang", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("foxfang-update");
    const localAppData = createCaseDir("foxfang-localappdata");
    ...
    const portableGitUsr = path.join(localAppData, "FoxFang", "deps", "portable-git", "usr", "bin");
  ...
  it.each([
    ...
    },
    {
      ...
      invoke: async () => {
        await withEnvAsync(
          {
            FOXFANG_STATE_DIR: "./state",
            FOXFANG_CONFIG_PATH: "./config/foxfang.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            FOXFANG_STATE_DIR: path.resolve("./state"),
            FOXFANG_CONFIG_PATH: path.resolve("./config/foxfang.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            FOXFANG_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ FOXFANG_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.FOXFANG_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("foxfang-update-wizard");
    await withEnvAsync({ FOXFANG_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/foxfang as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ FOXFANG_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "foxfang"));
    });

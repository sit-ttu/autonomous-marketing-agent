...
export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["foxfang", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.FOXFANG_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "foxfang");
  }
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveFoxFangPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", FOXFANG_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `FOXFANG_GIT_DIR points at a non-git directory: ${params.dir}. Set FOXFANG_GIT_DIR to an empty folder or an foxfang checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`FOXFANG_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }

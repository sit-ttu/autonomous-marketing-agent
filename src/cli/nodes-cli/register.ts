    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["foxfang nodes status", "List known nodes with live status."],
          ["foxfang nodes pairing pending", "Show pending node pairing requests."],
          ['foxfang nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["foxfang nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.foxfang.ai/cli/nodes")}\n`,
    );

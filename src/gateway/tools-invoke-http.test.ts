...
let lastCreateFoxFangToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/foxfang-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateFoxFangToolsContext?.agentTo,
          agentThreadId: lastCreateFoxFangToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createFoxFangTools: (ctx: Record<string, unknown>) => {
      lastCreateFoxFangToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateFoxFangToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateFoxFangToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateFoxFangToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-foxfang-message-to": "channel:24514",
        "x-foxfang-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });

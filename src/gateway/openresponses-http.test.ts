...
async function writeGatewayConfig(config: Record<string, unknown>) {
  ...
  if (!configPath) {
    throw new Error("FOXFANG_CONFIG_PATH is required for gateway config tests");
  }
  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "foxfang", input: "hi" }),
      });
      ...
      expect(invalidModelJson.error?.message).toBe(
        "Invalid `model`. Use `foxfang` or `foxfang/<agentId>`.",
      );
      ...
      const resHeader = await postResponses(
        port,
        { model: "foxfang", input: "hi" },
        { "x-foxfang-agent-id": "beta" },
      );
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "foxfang/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "foxfang/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "foxfang", input: "hi" },
        { "x-foxfang-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-foxfang-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);

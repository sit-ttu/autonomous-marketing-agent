...
import type { loadFoxFangPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureFoxFangModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadFoxFangPlugins>;
  ...
  // Tests can opt out via FOXFANG_SKIP_CHANNELS (or legacy FOXFANG_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.FOXFANG_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.FOXFANG_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (FOXFANG_SKIP_CHANNELS=1 or FOXFANG_SKIP_PROVIDERS=1)",
    );
  }

...
type GatewaySecretDefaults = NonNullable<FoxFangConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${FOXFANG_GATEWAY_TOKEN}` from being
...
export function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.FOXFANG_GATEWAY_TOKEN);
}

export function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  return trimToUndefined(env.FOXFANG_GATEWAY_PASSWORD);
}

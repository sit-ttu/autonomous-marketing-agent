...
const ensureFoxFangModelsJsonMock = vi.fn<
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveFoxFangAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureFoxFangModelsJson: (config: unknown, agentDir: unknown) =>
    ensureFoxFangModelsJsonMock(config, agentDir),
}));
...
describe("gateway startup primary model warmup", () => {
  beforeEach(() => {
    ensureFoxFangModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureFoxFangModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureFoxFangModelsJsonMock).not.toHaveBeenCalled();

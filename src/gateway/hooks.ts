...
export function resolveHooksConfig(cfg: FoxFangConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: FoxFangConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-foxfang-token"] === "string" ? req.headers["x-foxfang-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-foxfang-idempotency-key"]) ||

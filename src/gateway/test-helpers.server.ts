...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "FOXFANG_STATE_DIR",
  "FOXFANG_CONFIG_PATH",
  "FOXFANG_SKIP_BROWSER_CONTROL_SERVER",
  "FOXFANG_SKIP_GMAIL_WATCHER",
  "FOXFANG_SKIP_CANVAS_HOST",
  "FOXFANG_BUNDLED_PLUGINS_DIR",
  "FOXFANG_SKIP_CHANNELS",
  "FOXFANG_SKIP_PROVIDERS",
  "FOXFANG_SKIP_CRON",
  "FOXFANG_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.FOXFANG_CONFIG_PATH) {
    configPaths.add(process.env.FOXFANG_CONFIG_PATH);
  }
  if (process.env.FOXFANG_STATE_DIR) {
    configPaths.add(path.join(process.env.FOXFANG_STATE_DIR, "foxfang.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.FOXFANG_STATE_DIR = path.join(tempHome, ".foxfang");
  delete process.env.FOXFANG_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.FOXFANG_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.FOXFANG_SKIP_GMAIL_WATCHER = "1";
  process.env.FOXFANG_SKIP_CANVAS_HOST = "1";
  process.env.FOXFANG_SKIP_CHANNELS = "1";
  process.env.FOXFANG_SKIP_PROVIDERS = "1";
  process.env.FOXFANG_SKIP_CRON = "1";
  process.env.FOXFANG_TEST_MINIMAL_GATEWAY = "1";
  process.env.FOXFANG_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "foxfang-test-no-bundled-extensions")
    : "foxfang-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  const stateDir = process.env.FOXFANG_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".foxfang-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".foxfang-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".foxfang-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>foxfang-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__foxfangTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__foxfangTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["FOXFANG_GATEWAY_TOKEN"]);
  const prev = process.env.FOXFANG_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.FOXFANG_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.FOXFANG_STATE_DIR ?? process.env.HOME ?? os.tmpdir();

...
function cfg(input: Partial<FoxFangConfig>): FoxFangConfig {
  return input as FoxFangConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: FoxFangConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
...
const DEFAULT_ENV = {
  FOXFANG_GATEWAY_TOKEN: "env-token",
  FOXFANG_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when FOXFANG env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when FOXFANG env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when FOXFANG env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {

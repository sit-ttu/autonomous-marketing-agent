...
import { ensureFoxFangCliOnPath } from "../infra/path-env.js";
import {
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("foxfang doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.FOXFANG_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.FOXFANG_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "FOXFANG_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "FOXFANG_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `foxfang config set gateway.auth.mode token` and `foxfang config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  if (matrixInstallPathIssue) {
    const lines = formatPluginInstallPathIssue({
      ...
      pluginLabel: "Matrix",
      defaultInstallCommand: "foxfang plugins install @foxfang/matrix",
      repoInstallCommand: "foxfang plugins install ./extensions/matrix",
  ...
  if (controlUiRootOverride) {
    ...
    if (!resolvedRoot) {
      const ensureResult = await ensureControlUiAssetsBuilt(gatewayRuntime, { allowBuild: false });
  ...
  try {
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachFoxFangTranscriptMeta(update.message, {
    ...
    if (!minimalTestGateway) {
      void (async () => {
        ...
        logDiscovery.warn(`gateway discovery failed: ${String(err)}`);
      });
    }
    ...
    if (!minimalTestGateway) {
      void (async () => {
        ...
        log.warn(`gateway sidecar startup failed: ${String(err)}`);
      });
    }

...
type GatewayInstallAuthMode = NonNullable<NonNullable<FoxFangConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: FoxFangConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.FOXFANG_GATEWAY_PASSWORD?.trim() ||

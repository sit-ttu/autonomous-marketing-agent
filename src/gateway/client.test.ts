...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("foxfang doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with FOXFANG_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with FOXFANG_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.FOXFANG_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://foxfang-gateway.ai:18789",
      onConnectError,
    });

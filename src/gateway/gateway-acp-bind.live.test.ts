...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.FOXFANG_LIVE_ACP_BIND);
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.FOXFANG_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.FOXFANG_LIVE_ACP_BIND_AGENT);
      const acpxCommand = process.env.FOXFANG_LIVE_ACP_BIND_ACPX_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "foxfang.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.FOXFANG_GATEWAY_PORT = previous.port;
        }

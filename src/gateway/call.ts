...
function loadGatewayConfig(): FoxFangConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: FoxFangConfig, env?: NodeJS.ProcessEnv): number {
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__FOXFANG_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;

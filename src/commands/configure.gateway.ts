...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "FOXFANG_GATEWAY_TOKEN",
          placeholder: "FOXFANG_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like FOXFANG_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. FoxFang will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: FoxFang trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.foxfang.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );

...
const SEARCH_TOOL = "https://docs.foxfang.ai/mcp.SearchFoxFang";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.foxfang.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('foxfang docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.foxfang.ai/");
      runtime.log(`Search: ${formatCliCommand('foxfang docs "your query"')}`);
    }
    return;
  }

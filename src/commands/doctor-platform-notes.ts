...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".foxfang", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: FoxFangConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [["FOXFANG_GATEWAY_TOKEN", await getenv("FOXFANG_GATEWAY_TOKEN")]] as const;
  const passwordEntries = [
    ["FOXFANG_GATEWAY_PASSWORD", await getenv("FOXFANG_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "FOXFANG_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = env.FOXFANG_NO_RESPAWN?.trim() ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- FOXFANG_NO_RESPAWN is not set to 1; set it to avoid extra startup overhead from self-respawn.",
    );
  }

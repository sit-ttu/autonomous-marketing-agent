  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "foxfang doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: FoxFangConfig): Promise<string[]> {

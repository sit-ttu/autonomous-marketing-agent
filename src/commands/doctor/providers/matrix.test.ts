...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "foxfang-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("foxfang plugins install @foxfang/matrix");
    expect(warnings[2]).toContain("foxfang plugins install ./extensions/matrix");
  });
  ...
  it("removes stale Matrix plugin config when install path is missing", async () => {
    const missingPath = path.join(tmpdir(), "foxfang-matrix-stale-cleanup-test-" + Date.now());
    await fs.rm(missingPath, { recursive: true, force: true });

...
import { FoxFangSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: FoxFangConfig): {
  config: FoxFangConfig;
  removed: string[];
} {
  const parsed = FoxFangSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: FoxFangConfig): void {

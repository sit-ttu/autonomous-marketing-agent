...
type EnvSnapshot = {
  HOME?: string;
  FOXFANG_HOME?: string;
  FOXFANG_STATE_DIR?: string;
  FOXFANG_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    FOXFANG_HOME: process.env.FOXFANG_HOME,
    FOXFANG_STATE_DIR: process.env.FOXFANG_STATE_DIR,
    FOXFANG_OAUTH_DIR: process.env.FOXFANG_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: FoxFangConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: FoxFangConfig) {
...
async function runStateIntegrityText(cfg: FoxFangConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "foxfang-doctor-state-integrity-"));
    process.env.HOME = tempHome;
    process.env.FOXFANG_HOME = tempHome;
    process.env.FOXFANG_STATE_DIR = path.join(tempHome, ".foxfang");
    delete process.env.FOXFANG_OAUTH_DIR;
    fs.mkdirSync(process.env.FOXFANG_STATE_DIR, { recursive: true, mode: 0o700 });
  ...
  it("prompts for oauth dir when FOXFANG_OAUTH_DIR is explicitly configured", async () => {
    process.env.FOXFANG_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: FoxFangConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
    expect(stateIntegrityText()).toContain("CRITICAL: OAuth dir missing");
  ...
  it("prints foxfang-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: FoxFangConfig = {};
    ...
    expect(text).toMatch(/foxfang sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/foxfang sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /foxfang sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );

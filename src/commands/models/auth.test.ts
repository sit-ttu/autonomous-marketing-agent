...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: FoxFangConfig;
  let lastUpdatedConfig: FoxFangConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/foxfang/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/foxfang/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/foxfang/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: FoxFangConfig) => FoxFangConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/foxfang/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/foxfang/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `foxfang plugins list --json`.',
    );
  });

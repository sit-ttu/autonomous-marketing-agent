...
export async function loadValidConfigOrThrow(): Promise<FoxFangConfig> {
...
export async function updateConfig(
  mutator: (cfg: FoxFangConfig) => FoxFangConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: FoxFangConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: FoxFangConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("foxfang agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}

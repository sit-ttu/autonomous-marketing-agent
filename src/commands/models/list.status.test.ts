...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveFoxFangAgentDir: vi.fn().mockReturnValue("/tmp/foxfang-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/foxfang-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/foxfang-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/foxfang-dev/foxfang.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveFoxFangAgentDir: mocks.resolveFoxFangAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/foxfang-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveFoxFangAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/foxfang-dev/foxfang.json");
    expect(payload.auth.storePath).toBe("/tmp/foxfang-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/foxfang-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/foxfang-agent-custom");

...
import { readConfigFileSnapshot, writeConfigFile, type FoxFangConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("foxfang config set gateway.auth.mode token")} or ${formatCliCommand("foxfang config set gateway.auth.mode password")}.`,
  ].join(" ");
}

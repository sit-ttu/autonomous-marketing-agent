...
function createPerplexityConfig(apiKey: string, enabled?: boolean): FoxFangConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: FoxFangConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): FoxFangConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: FoxFangConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: FoxFangConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);

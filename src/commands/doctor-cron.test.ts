...
async function makeTempStorePath() {
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-doctor-cron-"));
...
function createCronConfig(storePath: string): FoxFangConfig {
  return {
    cron: {
      store: storePath,
...
describe("maybeRepairLegacyCronStore", () => {
  ...
  it("warns and continues when the cron job store cannot be read", async () => {
    ...
    // directory where the cron job store file would be.
    await fs.mkdir(path.dirname(storePath), { recursive: true });
    ...
    expect(noteSpy).toHaveBeenCalledWith(
      expect.stringContaining("Unable to read cron job store at"),
      "Cron",
    );
    expect(noteSpy).toHaveBeenCalledWith(
      expect.stringContaining("later health checks will continue"),
      "Cron",
    );
  });
});

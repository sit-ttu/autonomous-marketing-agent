...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.FOXFANG_DEBUG_HEALTH)) {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: FoxFangConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: FoxFangConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.FOXFANG_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {

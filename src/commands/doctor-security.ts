...
import type { FoxFangConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: FoxFangConfig): string[] {
...
export async function noteSecurityWarnings(cfg: FoxFangConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("foxfang security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.foxfang/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("foxfang approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("foxfang configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("foxfang config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("foxfang doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("foxfang config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('foxfang config set session.dmScope "per-channel-peer"') +

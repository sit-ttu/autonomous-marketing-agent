...
import { type FoxFangConfig, readConfigFileSnapshot } from "../config/config.js";
...
export async function requireValidConfigSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("foxfang doctor")}.`);
    runtime.exit(1);
    return null;
  }
  ...
  if (compatibility.length > 0) {
    runtime.log(
      [
        ...
        `Review: ${formatCliCommand("foxfang doctor")}`,
      ].join("\n"),
    );
  }
  return snapshot.config;

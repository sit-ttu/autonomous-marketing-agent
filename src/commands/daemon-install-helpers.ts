...
export async function buildGatewayInstallPlan(params: {
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      process.platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(params.env.FOXFANG_PROFILE)
        : undefined,
  ...
  //   1. ~/.foxfang/.env file vars  (lowest — user secrets / fallback keys)
  //   2. Config env vars              (foxfang.json env.vars + inline keys)
  //   3. Auth-profile env refs        (credential store → env var lookups)
  //   4. Service environment          (HOME, PATH, FOXFANG_* — highest)
  return {
    programArguments,
    workingDirectory,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("foxfang gateway install")}\` after fixing the error.`;
}

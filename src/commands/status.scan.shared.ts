...
export function hasExplicitMemorySearchConfig(cfg: FoxFangConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: FoxFangConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: FoxFangConfig, agentId: string) => { store: { path: string } } | null;

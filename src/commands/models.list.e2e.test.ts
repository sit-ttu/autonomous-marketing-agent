...
const ensureFoxFangModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveFoxFangAgentDir = vi.fn().mockReturnValue("/tmp/foxfang-agent");
...
beforeEach(() => {
  ...
  ensureFoxFangModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureFoxFangModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureFoxFangModelsJson).toHaveBeenCalled();
    expect(ensureFoxFangModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });

...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under FOXFANG_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("foxfang-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-oauth-sync-"));
    ...
    process.env.FOXFANG_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-oauth-nosync-"));
    process.env.FOXFANG_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside FOXFANG_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-oauth-external-"));
    process.env.FOXFANG_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* FOXFANG_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to FOXFANG_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("foxfang-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);

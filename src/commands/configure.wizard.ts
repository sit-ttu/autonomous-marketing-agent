...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.FOXFANG_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.FOXFANG_GATEWAY_PASSWORD ?? configuredPassword;
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from foxfang.json",
        },
      ],
...
export async function runConfigureWizard(
  ...
  try {
    intro(opts.command === "update" ? "FoxFang update wizard" : "FoxFang configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: FoxFangConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.foxfang.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("foxfang doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: process.env.FOXFANG_GATEWAY_TOKEN ?? baseLocalProbeToken,
      password: process.env.FOXFANG_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    });

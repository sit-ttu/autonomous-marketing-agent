...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.foxfang/workspace",
...
describe("setupWizardCommand", () => {
  ...
  it("prints the FoxFang ASCII banner before setup", async () => {
    const runtime = makeRuntime();

    await setupWizardCommand({}, runtime);

    expect(mocks.printWizardHeader).toHaveBeenCalledWith(runtime);
  });
  ...
  it("logs ASCII-safe Windows guidance before setup", async () => {
    ...
    try {
      ...
      expect(runtime.log).toHaveBeenCalledWith(
        [
          "Windows detected - FoxFang runs great on WSL2!",
          "Native Windows might be trickier.",
          "Quick setup: wsl --install (one command, one reboot)",
          "Guide: https://docs.foxfang.ai/windows",
        ].join("\n"),
      );
    } finally {
  ...
  it("uses configured default workspace for --reset when --workspace is not provided", async () => {
    ...
    mocks.readConfigFileSnapshot.mockResolvedValue({
      ...
      config: {
        agents: {
          defaults: {
            workspace: "/tmp/foxfang-custom-workspace",
          },
        },
      },
    });
    ...
    expect(mocks.handleReset).toHaveBeenCalledWith(
      "config+creds+sessions",
      path.resolve("/tmp/foxfang-custom-workspace"),
      runtime,
    );
  });

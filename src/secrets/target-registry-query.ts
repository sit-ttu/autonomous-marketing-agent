...
const FOXFANG_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
  (entry) => entry.configFile === "foxfang.json",
);
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of FOXFANG_COMPILED_SECRET_TARGETS) {
...
const FOXFANG_TARGETS_BY_ID = buildConfigTargetIdIndex();
...
export function discoverConfigSecretTargets(config: FoxFangConfig): DiscoveredConfigSecretTarget[] {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: FOXFANG_COMPILED_SECRET_TARGETS,
    entriesById: FOXFANG_TARGETS_BY_ID,
  });

...
export async function runSetupWizard(
  ...
  await prompter.intro("FoxFang setup");

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: FoxFangConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("foxfang doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("foxfang configure")}.`;
  ...
  let localGatewayToken = process.env.FOXFANG_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.FOXFANG_GATEWAY_PASSWORD;
  try {

...
export async function finalizeSetupWizard(
  ...
  const FAST_GATEWAY_PROBE_MS = 400;
  ...
  if (!opts.skipHealth) {
    gatewayProbe = installDaemon
      ...
      : await probeGatewayReachable({
          url: probeLinks.wsUrl,
          ...probeAuth,
          timeoutMs: FAST_GATEWAY_PROBE_MS,
        });
    if (gatewayProbe.ok && installDaemon) {
      try {
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.foxfang.ai/gateway/health",
            "https://docs.foxfang.ai/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("foxfang gateway run")}`,
          `Or rerun with: ${formatCliCommand("foxfang onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("foxfang onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.foxfang.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.foxfang/foxfang.json (gateway.auth.token) or FOXFANG_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("foxfang config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("foxfang doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("foxfang dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control FoxFang."
            : "Copy/paste this URL in a browser on this machine to control FoxFang.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("foxfang dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control FoxFang."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control FoxFang.",
  );

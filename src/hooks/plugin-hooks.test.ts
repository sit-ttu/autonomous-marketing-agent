...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "foxfang-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.FOXFANG_BUNDLED_HOOKS_DIR;
    process.env.FOXFANG_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });

  afterEach(() => {
    clearInternalHooks();
    if (previousBundledHooksDir === undefined) {
      delete process.env.FOXFANG_BUNDLED_HOOKS_DIR;
    } else {
      process.env.FOXFANG_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  function createConfig(enabled: boolean): FoxFangConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("foxfang-plugin");
  ...
  it("does not treat Claude hooks.json bundles as FoxFang hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".foxfang", "extensions", "claude-bundle");

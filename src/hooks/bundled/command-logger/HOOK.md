...
homepage: https://docs.foxfang.ai/automation/hooks#command-logger
metadata:
  {
    "foxfang":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.foxfang/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.foxfang/logs/commands.log ~/.foxfang/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/foxfang`:
   ```
   /home/username/.foxfang/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok

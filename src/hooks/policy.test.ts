...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "foxfang-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "foxfang-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "foxfang-bundled");
      const plugin = makeHookEntry("shared", "foxfang-plugin");
      const managed = makeHookEntry("shared", "foxfang-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("foxfang-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "foxfang-managed");
      const workspace = makeHookEntry("shared", "foxfang-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "foxfang-workspace");
      const second = makeHookEntry("shared", "foxfang-workspace");
      second.hook.handlerPath = "/tmp/foxfang-workspace/shared/handler-2.js";

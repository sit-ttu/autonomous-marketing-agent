...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.foxfangCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.foxfangCanvasA2UIAction &&
        typeof window.foxfangCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.foxfangSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("foxfang:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android FoxFang node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.foxfangSendUserAction === "function"
        ? window.foxfangSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.FOXFANG_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.FOXFANG_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>FoxFang Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;

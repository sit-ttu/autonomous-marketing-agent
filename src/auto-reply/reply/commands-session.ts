...
import { scheduleGatewaySigusr1Restart, triggerFoxFangRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting FoxFang in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerFoxFangRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting FoxFang via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};

...
type HomeEnvSnapshot = {
  ...
  FOXFANG_STATE_DIR: string | undefined;
  FOXFANG_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    FOXFANG_AGENT_DIR: process.env.FOXFANG_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".foxfang", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.FOXFANG_STATE_DIR = path.join(home, ".foxfang");
    process.env.FOXFANG_AGENT_DIR = path.join(home, ".foxfang", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".foxfang", "agent");

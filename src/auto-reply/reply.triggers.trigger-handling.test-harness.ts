...
const piEmbeddedMocks = getSharedMocks("foxfang.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("foxfang.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("foxfang.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("foxfang.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    foxfangHome: process.env.FOXFANG_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("FOXFANG_HOME", snapshot.foxfangHome);
  restoreKey("FOXFANG_STATE_DIR", snapshot.stateDir);
}

function setTempHomeEnv(home: string): void {
  process.env.HOME = home;
  process.env.USERPROFILE = home;
  delete process.env.FOXFANG_HOME;
  process.env.FOXFANG_STATE_DIR = join(home, ".foxfang");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "foxfang-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".foxfang", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): FoxFangConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "foxfang"),

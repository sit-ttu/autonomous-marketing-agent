...
export function isCommandEnabled(cfg: FoxFangConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: FoxFangConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: FoxFangConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: FoxFangConfig) {

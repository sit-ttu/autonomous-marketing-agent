...
function listAgents(cfg: FoxFangConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: FoxFangConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<FoxFangConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: FoxFangConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: FoxFangConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: FoxFangConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<FoxFangConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  FoxFangConfig,
  {
    bindingsRef: FoxFangConfig["bindings"];
    agentsRef: FoxFangConfig["agents"];
    sessionRef: FoxFangConfig["session"];
...
function resolveRouteCacheForConfig(cfg: FoxFangConfig): Map<string, ResolvedAgentRoute> {

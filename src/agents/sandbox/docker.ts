...
import { markFoxFangExecEnv } from "../../infra/foxfang-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "foxfang.sandbox=1");
  args.push("--label", `foxfang.sessionKey=${params.scopeKey}`);
  args.push("--label", `foxfang.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `foxfang.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markFoxFangExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "foxfang.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`foxfang sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`foxfang sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("foxfang sandbox recreate --all");
}

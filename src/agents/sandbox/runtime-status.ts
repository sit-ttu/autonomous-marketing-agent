...
function resolveMainSessionKeyForSandbox(params: { cfg?: FoxFangConfig; agentId: string }): string {
...
export function resolveSandboxRuntimeStatus(params: { cfg?: FoxFangConfig; sessionKey?: string }): {
  agentId: string;
  sessionKey: string;
...
export function formatSandboxToolPolicyBlockedMessage(params: {
  ...
  const explainCommand = runtime.sessionKey
    ? hasUnsafeControlChars(runtime.sessionKey)
      ? `foxfang sandbox explain --agent ${runtime.agentId}`
      : `foxfang sandbox explain --session ${shellEscapeSingleArg(runtime.sessionKey)}`

...
import { createFoxFangTools } from "./foxfang-tools.js";
...
function applyModelProviderToolPolicy(
  ...
  // Models with a native web_search tool cannot receive FoxFang's
...
function resolveExecConfig(params: { cfg?: FoxFangConfig; agentId?: string }) {
...
export function createFoxFangCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createFoxFangReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });

...
function resolveAttachmentLimits(config: FoxFangConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".foxfang", "attachments");
  const relDir = path.posix.join(".foxfang", "attachments", attachmentId);

 ...
 * A common mistake — caused by the similarity with the `foxfang.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (foxfang.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.FOXFANG_AUTH_STORE_READONLY === "1";

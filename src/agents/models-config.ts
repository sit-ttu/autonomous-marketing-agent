...
import { planFoxFangModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: FoxFangConfig): {
  config: FoxFangConfig;
  sourceConfigForSecrets: FoxFangConfig;
} {
...
export async function ensureFoxFangModelsJson(
  config?: FoxFangConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveFoxFangAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planFoxFangModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,

...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".foxfang", "source-origin.json");
...
function resolveSkillsLimits(config?: FoxFangConfig): ResolvedSkillsLimits {
...
function resolveSkillEntryMetadata(params: {
  ...
  skillDir: string;
}): FoxFangSkillMetadata | undefined {
  const metadata = resolveFoxFangMetadata(params.frontmatter);
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "foxfang-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "foxfang-managed",
  });
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: FoxFangConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`foxfang skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`foxfang skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,

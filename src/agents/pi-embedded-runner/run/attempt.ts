...
import { resolveFoxFangAgentDir } from "../../agent-paths.js";
...
import { createFoxFangCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  log.info(
    `[agent] ▶ start runId=${params.runId} sessionId=${params.sessionId} provider=${params.provider} model=${params.modelId} thinking=${params.thinkLevel} channel=${params.messageChannel ?? params.messageProvider ?? "unknown"} sessionKey=${params.sessionKey ?? "-"} trigger=${params.trigger ?? "unknown"} workspace=${resolvedWorkspace}`,
  );
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveFoxFangAgentDir();
    ...
    try {
      ...
      let streamFnSource: string;
      if (providerStreamFn) {
        activeSession.agent.streamFn = providerStreamFn;
        streamFnSource = `plugin:custom(${params.provider})`;
      } else if (
        shouldUseOpenAIWebSocketTransport({
          provider: params.provider,
          modelApi: params.model.api,
        ...
        if (wsApiKey) {
          ...
          streamFnSource = `openai-websocket(${params.provider})`;
        } else {
          ...
          streamFnSource = `default(${params.provider}, no ws key)`;
        }
        ...
        streamFnSource = "anthropic-vertex";
      } else {
        activeSession.agent.streamFn = defaultSessionStreamFn;
        streamFnSource = `default(${params.provider})`;
      }
      log.info(
        `[agent] ⚙ streamFn selected: runId=${params.runId} source=${streamFnSource} transport=${activeSession.agent.transport ?? "default"}`,
      );
      ...
      try {
        ...
        // double-compaction. See: https://github.com/PotLock/foxfang/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("foxfang:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/PotLock/foxfang/issues/8643

...
describe("createFoxFangCodingTools message provider policy", () => {
  it.each(["voice", "VOICE", " Voice "])(
    "does not expose tts tool for normalized voice provider: %s",
    (messageProvider) => {
      const tools = createFoxFangCodingTools({ messageProvider });
  ...
  it("keeps tts tool for non-voice providers", () => {
    const tools = createFoxFangCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));

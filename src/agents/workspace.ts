...
export function resolveDefaultAgentWorkspaceDir(
  ...
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".foxfang", `workspace-${profile}`);
  }
  return path.join(home, ".foxfang", "workspace");
}
...
export const DEFAULT_BRAND_FILENAME = "BRAND.md";
export const DEFAULT_PRODUCT_FILENAME = "PRODUCT.md";
const WORKSPACE_STATE_DIRNAME = ".foxfang";
...
const MEMORY_SCAFFOLD_FALLBACK = [
  "# MEMORY.md - Long-Term Memory",
  "",
  "_Curated memory for main (private) sessions. Distill durable facts, preferences, and lessons from memory/YYYY-MM-DD.md._",
  "",
  "## Brand and positioning",
  "",
  "## Campaigns and content",
  "",
  "## Audience and channel learnings",
  "",
  "## Open loops",
  "",
].join("\n");

async function ensureWorkspaceMemoryScaffold(dir: string): Promise<void> {
  await fs.mkdir(path.join(dir, DEFAULT_MEMORY_DIRNAME), { recursive: true });

  const memoryPath = path.join(dir, DEFAULT_MEMORY_FILENAME);
  let memoryTemplate = MEMORY_SCAFFOLD_FALLBACK;
  try {
    memoryTemplate = await loadTemplate(DEFAULT_MEMORY_FILENAME);
  } catch {
    // Template dir may be unavailable in some test/dev layouts; fallback is enough.
  }
  await writeFileIfMissing(memoryPath, memoryTemplate);
}

const BRAND_SCAFFOLD_FALLBACK = [
  "# BRAND.md — Brand Context",
  "",
  "_Fill in brand voice, positioning, audience, and guardrails._",
  "",
].join("\n");
const PRODUCT_SCAFFOLD_FALLBACK = [
  "# PRODUCT.md — Product Intelligence",
  "",
  "_Fill in product facts, benefits, USPs, and FAQs._",
  "",
].join("\n");

async function loadTemplateOrFallback(name: string, fallback: string): Promise<string> {
  try {
...
async function ensureWorkspaceMarketingScaffold(dir: string): Promise<void> {
  const brandPath = path.join(dir, DEFAULT_BRAND_FILENAME);
  const productPath = path.join(dir, DEFAULT_PRODUCT_FILENAME);
  const brandTemplate = await loadTemplateOrFallback(
    DEFAULT_BRAND_FILENAME,
    BRAND_SCAFFOLD_FALLBACK,
  );
  const productTemplate = await loadTemplateOrFallback(
    DEFAULT_PRODUCT_FILENAME,
    PRODUCT_SCAFFOLD_FALLBACK,
  );
  await writeFileIfMissing(brandPath, brandTemplate);
  await writeFileIfMissing(productPath, productTemplate);
}
...
export async function ensureAgentWorkspace(params?: {
  ...
  const isBrandNewWorkspace = await (async () => {
    ...
    const userContentPaths = [
      path.join(dir, DEFAULT_MEMORY_DIRNAME),
      path.join(dir, DEFAULT_MEMORY_FILENAME),
      path.join(dir, DEFAULT_BRAND_FILENAME),
      path.join(dir, DEFAULT_PRODUCT_FILENAME),
      path.join(dir, ".git"),
    ];
  ...
  await ensureWorkspaceMemoryScaffold(dir);
  await ensureWorkspaceMarketingScaffold(dir);

...
export function handleAgentStart(ctx: EmbeddedPiSubscribeContext) {
  ...
  const messageCount = ctx.state.assistantTexts.length;
  ctx.log.info(
    `[agent] ◀ agent_start runId=${ctx.params.runId} sessionId=${ctx.params.sessionId} sessionKey=${ctx.params.sessionKey ?? "-"} priorAssistantTurns=${messageCount}`,
  );
  emitAgentEvent({
...
export function handleAgentEnd(ctx: EmbeddedPiSubscribeContext) {
  ...
  if (isError && lastAssistant) {
    ...
    const lastAssistant = ctx.state.lastAssistant;
    const stopReason =
      lastAssistant && isAssistantMessage(lastAssistant) ? (lastAssistant.stopReason ?? "?") : "?";
    const usage = ctx.getUsageTotals?.();
    const usageSummary = usage
      ? `in=${usage.input ?? 0} out=${usage.output ?? 0} cacheRead=${usage.cacheRead ?? 0} cacheWrite=${usage.cacheWrite ?? 0}`
      : "n/a";
    ctx.log.info(
      `[agent] ■ agent_end runId=${ctx.params.runId} isError=${isError} stopReason=${stopReason} usage=${usageSummary} compactions=${ctx.getCompactionCount?.() ?? 0}`,
    );
    emitAgentEvent({
      runId: ctx.params.runId,
      stream: "lifecycle",

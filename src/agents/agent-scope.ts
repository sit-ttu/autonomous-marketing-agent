...
type AgentEntry = NonNullable<NonNullable<FoxFangConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: FoxFangConfig): AgentEntry[] {
...
export function listAgentIds(cfg: FoxFangConfig): string[] {
...
export function resolveDefaultAgentId(cfg: FoxFangConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: FoxFangConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: FoxFangConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: FoxFangConfig, agentId: string) {
  const id = normalizeAgentId(agentId);

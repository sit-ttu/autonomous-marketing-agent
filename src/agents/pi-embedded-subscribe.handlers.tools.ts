...
const TOOL_LOG_ARGS_MAX_CHARS = 240;

function summarizeArgsForLog(args: unknown): string {
  if (args === undefined) {
    return "{}";
  }
  ...
  if (serialized.length > TOOL_LOG_ARGS_MAX_CHARS) {
    return `${serialized.slice(0, TOOL_LOG_ARGS_MAX_CHARS)}…(+${serialized.length - TOOL_LOG_ARGS_MAX_CHARS} chars)`;
  }
  return serialized;
}

const TOOL_LOG_RESULT_MAX_CHARS = 200;

function summarizeResultForLog(result: unknown): string {
  ...
  if (serialized.length > TOOL_LOG_RESULT_MAX_CHARS) {
    return `${serialized.slice(0, TOOL_LOG_RESULT_MAX_CHARS)}…(+${serialized.length - TOOL_LOG_RESULT_MAX_CHARS} chars)`;
  }
  return serialized;
}
...
export async function handleToolExecutionStart(
  ...
  ctx.log.info(
    `[agent] 🔧 tool_start runId=${ctx.params.runId} tool=${toolName} toolCallId=${toolCallId} meta=${meta ?? "-"} args=${argsPreview}`,
  );
...
export async function handleToolExecutionEnd(
  ...
  const resultPreview = summarizeResultForLog(sanitizedResult ?? result);
  ctx.log.info(
    `[agent] ✅ tool_end runId=${ctx.params.runId} tool=${toolName} toolCallId=${toolCallId}` +
      ` durationMs=${toolDurationMs ?? "?"} isError=${isToolError}` +

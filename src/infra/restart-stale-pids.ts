 ...
 * returning control to the caller (typically `triggerFoxFangRestart` →
...
/**
 * Parse foxfang gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("foxfang")) {
 ...
 * Returns only PIDs that belong to foxfang gateway processes (not the current process).
 */

...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: FoxFangConfig | null = null;
  ...
  get(key: string, cfg: FoxFangConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: FoxFangConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: FoxFangConfig): void {

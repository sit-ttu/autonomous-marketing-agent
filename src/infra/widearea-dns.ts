...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.FOXFANG_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*foxfang-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "foxfang";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "foxfang");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "foxfang-gw");
  ...
  records.push(`_foxfang-gw._tcp IN PTR ${instanceLabel}._foxfang-gw._tcp`);
  records.push(`${instanceLabel}._foxfang-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._foxfang-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; foxfang-content-hash: ${contentHash}\n${contentBody}`;
}

...
import { resolveFoxFangPackageRoot, resolveFoxFangPackageRootSync } from "./foxfang-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveFoxFangPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "foxfang" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/foxfang -> .../dist/index.js)
...
export async function ensureControlUiAssetsBuilt(
  ...
  opts?: { timeoutMs?: number; allowBuild?: boolean },
  ...
  if (opts?.allowBuild === false) {
    return {
      ok: false,
      built: false,
      message: `Missing Control UI assets at ${indexPath}. Build them with \`pnpm ui:build\` (auto-installs UI deps).`,
    };
  }

...
export const POSIX_FOXFANG_TMP_DIR = "/tmp/foxfang";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredFoxFangTmpDirOptions = {
...
export function resolvePreferredFoxFangTmpDir(
  options: ResolvePreferredFoxFangTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "foxfang" : `foxfang-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[foxfang] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback FoxFang temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback FoxFang temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_FOXFANG_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_FOXFANG_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_FOXFANG_TMP_DIR)) {
      return POSIX_FOXFANG_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_FOXFANG_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_FOXFANG_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_FOXFANG_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_FOXFANG_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_FOXFANG_TMP_DIR;

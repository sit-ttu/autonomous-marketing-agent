...
const PRIMARY_PACKAGE_NAME = "foxfang";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const FOXFANG_MAIN_PACKAGE_SPEC = "github:foxfang/foxfang#main";
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "FoxFang", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.FOXFANG_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.FOXFANG_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return FOXFANG_MAIN_PACKAGE_SPEC;
  }

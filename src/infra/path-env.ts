...
function candidateBinDirs(opts: EnsureFoxFangPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `foxfang` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "foxfang");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.FOXFANG_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "foxfang"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `foxfang`
...
/**
 * Best-effort PATH bootstrap so skills that require the `foxfang` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureFoxFangCliOnPath(opts: EnsureFoxFangPathOpts = {}) {
  if (isTruthyEnvValue(process.env.FOXFANG_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.FOXFANG_PATH_BOOTSTRAPPED = "1";

...
const DEFAULT_PACKAGE_NAME = "foxfang";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["foxfang", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "foxfang.mjs");
    ...
    const doctorStep = await runStep(
      step("foxfang doctor", doctorArgv, gitRoot, { FOXFANG_UPDATE_IN_PROGRESS: "1" }),
    );

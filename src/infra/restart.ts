...
export function triggerFoxFangRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.FOXFANG_SYSTEMD_UNIT,
      process.env.FOXFANG_PROFILE,
    );
  ...
  const label =
    process.env.FOXFANG_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.FOXFANG_PROFILE);

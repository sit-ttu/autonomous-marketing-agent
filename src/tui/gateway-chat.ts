...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set FOXFANG_GATEWAY_TOKEN/FOXFANG_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.FOXFANG_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.FOXFANG_GATEWAY_PASSWORD);

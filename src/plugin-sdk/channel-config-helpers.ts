...
function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`foxfang pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`foxfang pairing approve ${channelId} <code>`);
...
type ChannelConfigAccessorParams<Config extends FoxFangConfig = FoxFangConfig> = {
  cfg: Config;
  accountId?: string | null;
};
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends FoxFangConfig = FoxFangConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends FoxFangConfig = FoxFangConfig>(
...
> {
  const base = {
    resolveAllowFrom({ cfg, accountId }: { cfg: FoxFangConfig; accountId?: string | null }) {
...
function setTopLevelChannelEnabledInConfigSection<Config extends FoxFangConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends FoxFangConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends FoxFangConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {

...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when FOXFANG_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".foxfang", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when FOXFANG_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", FOXFANG_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".foxfang-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers FOXFANG_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        FOXFANG_PROFILE: "rescue",
        FOXFANG_STATE_DIR: "C:\\State\\foxfang",
      },
      expected: path.join("C:\\State\\foxfang", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", FOXFANG_PROFILE: "default" },
      expected: path.join("/home/test", ".foxfang", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem FoxFang Gateway",
          "cd /d C:\\Projects\\foxfang",
          "set NODE_ENV=production",
          "set FOXFANG_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\foxfang",
          environment: {
            NODE_ENV: "production",
            FOXFANG_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\foxfang\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\foxfang\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\FoxFang Share\\node.exe" "\\\\fileserver\\FoxFang Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\FoxFang Share\\node.exe",
            "\\\\fileserver\\FoxFang Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from FOXFANG_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ FOXFANG_STATE_DIR: path.join(tmpDir, "custom-state") }),

...
export class FoxFangChannelBridge {
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`foxfang mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `foxfang mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }

...
This document describes how the FoxFang ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`foxfang acp` exposes an ACP agent over stdio and forwards prompts to a running
FoxFang Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing FoxFang Gateway
...
- ACP client spawns `foxfang acp` and speaks ACP messages over stdio.
...
`foxfang acp` resolves the Gateway URL and auth from CLI flags or config:

...
export async function handleWhatsAppStart(host: FoxFangApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: FoxFangApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: FoxFangApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: FoxFangApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: FoxFangApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: FoxFangApp): string {
...
function resolveGatewayHttpAuthHeader(host: FoxFangApp): string | null {
...
function buildGatewayHttpHeaders(host: FoxFangApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: FoxFangApp) {
...
export function handleNostrProfileToggleAdvanced(host: FoxFangApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: FoxFangApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: FoxFangApp) {
  const state = host.nostrProfileFormState;

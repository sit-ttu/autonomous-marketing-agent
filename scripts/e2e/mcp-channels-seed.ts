...
async function main() {
  const stateDir = process.env.FOXFANG_STATE_DIR?.trim() || path.join(os.homedir(), ".foxfang");
  const configPath = process.env.FOXFANG_CONFIG_PATH?.trim() || path.join(stateDir, "foxfang.json");

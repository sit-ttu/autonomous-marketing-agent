...
INSTALL_URL="https://foxfang.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_FOXFANG_BIN="/opt/homebrew/bin/foxfang"
GUEST_FOXFANG_ENTRY="/opt/homebrew/lib/node_modules/foxfang/foxfang.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/foxfang-parallels-smoke.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/foxfang-parallels-build.lock"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://foxfang.ai/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: foxfang@2026.3.13-beta.1
...
spawn {*}$cmd
send -- "printf '__FOXFANG_READY__\\n'\r"
expect "__FOXFANG_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/foxfang-prl.sh <<'__FOXFANG_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__FOXFANG_SCRIPT__\r"
send -- "/bin/bash /tmp/foxfang-prl.sh; rc=\$?; rm -f /tmp/foxfang-prl.sh; printf '__FOXFANG_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
expect {
  -re {__FOXFANG_RC__:(-?[0-9]+)} {
...
resolve_latest_version() {
  ...
  npm view foxfang version --userconfig "$(mktemp)"
}
...
export FOXFANG_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/foxfang-install.sh
bash /tmp/foxfang-install.sh${version_arg_q}
$GUEST_FOXFANG_BIN --version
EOF
)"
}
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_FOXFANG_BIN" --version
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/foxfang-main-$short_head.tgz"
...
start_server() {
  ...
  ) >/tmp/foxfang-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/foxfang"; check_path "\$root/foxfang/extensions"; if [ -d "\$root/foxfang/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/foxfang/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
verify_gateway() {
  guest_current_user_cli "$GUEST_FOXFANG_BIN" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_current_user_cli "$GUEST_FOXFANG_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_cli "$GUEST_FOXFANG_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_cli "$GUEST_FOXFANG_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_cli \
    /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" \
    "$GUEST_FOXFANG_BIN" agent \
    --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_FOXFANG_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from foxfang dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/foxfang-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>FoxFang Control</title>' /tmp/foxfang-dashboard-smoke.html >/dev/null; then
      if grep -F '<foxfang-app></foxfang-app>' /tmp/foxfang-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>FoxFang Control</title>' /tmp/foxfang-dashboard-smoke.html >/dev/null
grep -F '<foxfang-app></foxfang-app>' /tmp/foxfang-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/foxfang-discord-token <<'__FOXFANG_TOKEN__'
$DISCORD_TOKEN_VALUE
__FOXFANG_TOKEN__
cat >/tmp/foxfang-discord-guilds.json <<'__FOXFANG_GUILDS__'
$guilds_json
__FOXFANG_GUILDS__
token="\$(tr -d '\n' </tmp/foxfang-discord-token)"
guilds_json="\$(cat /tmp/foxfang-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_FOXFANG_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_FOXFANG_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_FOXFANG_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_FOXFANG_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_FOXFANG_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_FOXFANG_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_FOXFANG_ENTRY channels status --probe --json
rm -f /tmp/foxfang-discord-token /tmp/foxfang-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "foxfang-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "foxfang-main-upgrade.tgz"

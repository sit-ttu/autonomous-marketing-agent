...
IMAGE_NAME="foxfang-openwebui-e2e"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
PROFILE_FILE="${FOXFANG_PROFILE_FILE:-$HOME/.profile}"
MODEL="${FOXFANG_OPENWEBUI_MODEL:-openai/gpt-5.4}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${FOXFANG_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${FOXFANG_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${FOXFANG_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${FOXFANG_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${FOXFANG_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="foxfang-openwebui-e2e-$$"
GW_NAME="foxfang-openwebui-gateway-$$"
OW_NAME="foxfang-openwebui-$$"
...
docker run -d \
  ...
  -e "FOXFANG_OPENWEBUI_MODEL=$MODEL" \
  ...
  bash -lc '
    ...
    node "$entry" config set gateway.auth.token "$FOXFANG_GATEWAY_TOKEN" >/dev/null
    node "$entry" config set gateway.http.endpoints.chatCompletions.enabled true --strict-json >/dev/null
    node "$entry" config set agents.defaults.model.primary "$FOXFANG_OPENWEBUI_MODEL" >/dev/null
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="FoxFang E2E" \
  -e WEBUI_SECRET_KEY="foxfang-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="FoxFang E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="foxfang/default" \
...
echo "Running Open WebUI -> FoxFang smoke..."
docker exec \

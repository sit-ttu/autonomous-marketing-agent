...
# FoxFang Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://foxfang.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one FoxFang."
...
GUM_VERSION="${FOXFANG_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 FoxFang Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://foxfang.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$FOXFANG_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.foxfang.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_foxfang_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.foxfang-* "$npm_root"/foxfang 2>/dev/null || true
}

extract_foxfang_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_foxfang_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/foxfang" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/foxfang"|"/usr/local/bin/foxfang")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/foxfang/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale foxfang symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing foxfang binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing FoxFang package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_foxfang_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "FoxFang npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*foxfang" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_foxfang_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_foxfang_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "FoxFang npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an foxfang binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${FOXFANG_TAGLINE_INDEX:-}" ]]; then
        if [[ "${FOXFANG_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((FOXFANG_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${FOXFANG_NO_ONBOARD:-0}
NO_PROMPT=${FOXFANG_NO_PROMPT:-0}
DRY_RUN=${FOXFANG_DRY_RUN:-0}
INSTALL_METHOD=${FOXFANG_INSTALL_METHOD:-}
FOXFANG_VERSION=${FOXFANG_VERSION:-latest}
USE_BETA=${FOXFANG_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/foxfang"
GIT_DIR=${FOXFANG_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${FOXFANG_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${FOXFANG_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${FOXFANG_VERBOSE:-0}"
VERIFY_INSTALL="${FOXFANG_VERIFY_INSTALL:-0}"
FOXFANG_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
FoxFang installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://foxfang.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/foxfang)
...
Environment variables:
  FOXFANG_INSTALL_METHOD=git|npm
  FOXFANG_VERSION=latest|next|main|<semver>|<spec>
  FOXFANG_BETA=0|1
  FOXFANG_GIT_DIR=...
  FOXFANG_GIT_UPDATE=0|1
  FOXFANG_NO_PROMPT=1
  FOXFANG_VERIFY_INSTALL=1
  FOXFANG_DRY_RUN=1
  FOXFANG_NO_ONBOARD=1
  FOXFANG_VERBOSE=1
  FOXFANG_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://foxfang.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://foxfang.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://foxfang.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://foxfang.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://foxfang.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected FoxFang checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a FoxFang source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_foxfang_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"foxfang"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://foxfang.ai/install.sh | bash"
}
...
ensure_foxfang_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/foxfang" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/foxfang" ]]; then
        ln -sf "$npm_root/foxfang/dist/entry.js" "${npm_bin}/foxfang"
        ui_info "Created foxfang bin link at ${npm_bin}/foxfang"
    fi
    return 0
}

# Check for existing FoxFang installation
check_existing_foxfang() {
    if [[ -n "$(type -P foxfang 2>/dev/null || true)" ]]; then
        ui_info "Existing FoxFang installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make foxfang show as \"command not found\" in new terminals."
...
warn_foxfang_not_found() {
    ui_warn "Installed, but foxfang is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t foxfang 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named foxfang; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_foxfang_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P foxfang 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/foxfang" ]]; then
        echo "${npm_bin}/foxfang"
        return 0
    fi
...
install_foxfang_from_git() {
    ...
    if [[ -d "$repo_dir/.git" ]]; then
        ...
        ui_info "Installing FoxFang from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning FoxFang" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building FoxFang" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/foxfang" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/foxfang"
    ui_success "FoxFang wrapper installed to \$HOME/.local/bin/foxfang"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view foxfang dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:foxfang/foxfang#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_foxfang() {
    local package_name="foxfang"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            FOXFANG_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="foxfang"
        else
            FOXFANG_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${FOXFANG_VERSION}" ]]; then
        FOXFANG_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${FOXFANG_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${FOXFANG_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing FoxFang v${resolved_version}"
    else
        ui_info "Installing FoxFang (${FOXFANG_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${FOXFANG_VERSION}")"

    if ! install_foxfang_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_foxfang_paths
        install_foxfang_npm "${install_spec}"
    fi

    if [[ "${FOXFANG_VERSION}" == "latest" && "${package_name}" == "foxfang" ]]; then
        if ! resolve_foxfang_bin &> /dev/null; then
            ui_warn "npm install foxfang@latest failed; retrying foxfang@next"
            cleanup_npm_foxfang_paths
            install_foxfang_npm "foxfang@next"
        fi
    fi

    ensure_foxfang_bin_link || true

    ui_success "FoxFang installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${FOXFANG_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_foxfang_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (foxfang not on PATH yet)"
        warn_foxfang_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${FOXFANG_PROFILE:-default}"
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${FOXFANG_CONFIG_PATH:-$HOME/.foxfang/foxfang.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run foxfang onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but foxfang not on PATH; skipping onboarding"
        warn_foxfang_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run foxfang onboard to retry"
        return
    }
}
...
resolve_foxfang_version() {
    local version=""
    local raw_version_output=""
    local claw="${FOXFANG_BIN:-}"
    if [[ -z "$claw" ]] && command -v foxfang &> /dev/null; then
        claw="$(command -v foxfang)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_foxfang_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/foxfang/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/foxfang/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: foxfang not on PATH yet"
        warn_foxfang_not_found
        return 1
    fi

    run_quiet_step "Checking FoxFang version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: foxfang gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_foxfang_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found FoxFang checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set FOXFANG_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_foxfang; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing FoxFang"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g foxfang &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g foxfang 2>/dev/null || true
        ...
        install_foxfang_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/foxfang" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/foxfang"
    ...
    FOXFANG_BIN="$(resolve_foxfang_bin || true)"
    ...
    installed_version=$(resolve_foxfang_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 FoxFang installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 FoxFang installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/foxfang"
        ui_kv "Update command" "foxfang update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://foxfang.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running foxfang doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                FOXFANG_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                FOXFANG_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                FOXFANG_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run foxfang doctor and foxfang plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run foxfang onboard later"
        else
            local config_path="${FOXFANG_CONFIG_PATH:-$HOME/.foxfang/foxfang.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (foxfang not on PATH yet)"
                    warn_foxfang_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run foxfang onboard to finish setup"
            return 0
        fi
    fi

    if command -v foxfang &> /dev/null; then
        local claw="${FOXFANG_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_foxfang_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (foxfang daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if FOXFANG_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: foxfang daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${FOXFANG_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi

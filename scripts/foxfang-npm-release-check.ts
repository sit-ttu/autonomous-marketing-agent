...
const EXPECTED_REPOSITORY_URL = "https://github.com/PotLock/foxfang";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "foxfang") {
    errors.push(`package.json name must be "foxfang"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.foxfang !== "foxfang.mjs") {
    errors.push(
      `package.json bin.foxfang must be "foxfang.mjs"; found "${pkg.bin?.foxfang ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`foxfang-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `foxfang-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}

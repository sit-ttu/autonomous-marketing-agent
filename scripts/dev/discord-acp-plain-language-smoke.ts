...
type DriverMode = "token" | "webhook" | "foxfang";
...
function resolveStateDir(): string {
  const override = process.env.FOXFANG_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.FOXFANG_HOME?.trim() || process.env.HOME || "";
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver foxfang] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) FoxFang spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|foxfang> Driver transport mode (default: token)\n" +
    ...
    "  --foxfang-bin <path>        FoxFang CLI binary for driver=foxfang (default: foxfang)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  FOXFANG_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  FOXFANG_DISCORD_SMOKE_DRIVER\n" +
    "  FOXFANG_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  FOXFANG_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  FOXFANG_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  FOXFANG_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  FOXFANG_DISCORD_SMOKE_AGENT\n" +
    "  FOXFANG_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  FOXFANG_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  FOXFANG_DISCORD_SMOKE_POLL_MS\n" +
    "  FOXFANG_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  FOXFANG_DISCORD_SMOKE_FOXFANG_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.FOXFANG_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.FOXFANG_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "foxfang"
        ? "foxfang"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken = resolveArg("--token") || process.env.FOXFANG_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.FOXFANG_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.FOXFANG_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") || process.env.FOXFANG_DISCORD_SMOKE_BOT_TOKEN_PREFIX || "Bot";
  const targetAgent = resolveArg("--agent") || process.env.FOXFANG_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.FOXFANG_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.FOXFANG_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.FOXFANG_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.FOXFANG_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.FOXFANG_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const foxfangBin =
    resolveArg("--foxfang-bin") || process.env.FOXFANG_DISCORD_SMOKE_FOXFANG_BIN || "foxfang";
...
async function foxfangCliJson<T>(params: { foxfangBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.foxfangBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`foxfang ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await foxfangCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    foxfangBin: params.foxfangBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "foxfang") {
    return await readMessagesWithOpenclaw({
      foxfangBin: params.args.foxfangBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `foxfang-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await foxfangCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("foxfang message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from FoxFang.`,
        diagnostics: {
          bindingCandidates: [
            {

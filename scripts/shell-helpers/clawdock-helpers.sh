#!/usr/bin/env bash
# ClawDock - Docker helpers for FoxFang
# Inspired by Simon Willison's "Running FoxFang in Docker"
# https://til.simonwillison.net/llms/foxfang-docker
#
# Installation:
...
# Common paths to check for FoxFang
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/foxfang"
  "${HOME}/workspace/foxfang"
  "${HOME}/projects/foxfang"
  "${HOME}/dev/foxfang"
  "${HOME}/code/foxfang"
  "${HOME}/src/foxfang"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found FoxFang at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/foxfang"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ FoxFang not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^FOXFANG_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d foxfang-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart foxfang-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f foxfang-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec foxfang-gateway \
    bash -c 'echo "alias foxfang=\"./foxfang.mjs\"" > /tmp/.bashrc_foxfang && bash --rcfile /tmp/.bashrc_foxfang'
}
...
clawdock-cli() {
  _clawdock_compose run --rm foxfang-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build foxfang-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "FOXFANG_GATEWAY_TOKEN=$token" foxfang-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" foxfang-gateway \
  ...
  local saved_token
  saved_token=$(_clawdock_compose exec foxfang-gateway \
  ...
  _clawdock_compose restart foxfang-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm foxfang-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec foxfang-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'foxfang config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for FoxFang${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (foxfang alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to foxfang project directory${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'foxfang channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'foxfang status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.foxfang.ai${_CLR_RESET}"
  echo ""
}

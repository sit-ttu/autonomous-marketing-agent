...
#
# Suggested cron: */30 * * * * /home/admin/foxfang/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send FoxFang notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., foxfang-alerts)
...
STATE_FILE="$HOME/.foxfang/auth-monitor-state"
...
send_notification() {
    ...
    # Send via FoxFang if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use foxfang
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via FoxFang to $NOTIFY_PHONE..."
            foxfang send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! FoxFang is down. Run: ssh l36 '~/foxfang/scripts/mobile-reauth.sh'" "urgent"
    exit 1

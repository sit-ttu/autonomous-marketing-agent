...
SMOKE_PREVIOUS_VERSION="${FOXFANG_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${FOXFANG_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="foxfang"
PACKAGE_NAME="${FOXFANG_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (FOXFANG_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${FOXFANG_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${FOXFANG_INSTALL_LATEST_OUT:-}"
fi

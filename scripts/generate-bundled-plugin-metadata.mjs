...
} from "./lib/bundled-plugin-source-utils.mjs";
...
function normalizePackageManifest(raw) {
  const packageManifest = normalizeObject(resolveBundledPackageManifest(raw));
  if (!packageManifest) {
    return undefined;
  }
  const normalized = {
...
function resolvePackageChannelMeta(packageJson) {
  const packageMeta = resolveBundledPackageManifest(packageJson);
  if (!packageMeta) {
    return undefined;
  }
  const channelMeta = packageMeta.channel;

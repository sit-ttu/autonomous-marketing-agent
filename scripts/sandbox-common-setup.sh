...
BASE_IMAGE="${BASE_IMAGE:-foxfang-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-foxfang-sandbox-common:bookworm-slim}"
...
FOXFANG_DOCKER_BUILD_USE_BUILDX="${FOXFANG_DOCKER_BUILD_USE_BUILDX:-0}"
FOXFANG_DOCKER_BUILD_CACHE_FROM="${FOXFANG_DOCKER_BUILD_CACHE_FROM:-}"
FOXFANG_DOCKER_BUILD_CACHE_TO="${FOXFANG_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${FOXFANG_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${FOXFANG_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${FOXFANG_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${FOXFANG_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${FOXFANG_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=foxfang.sandbox=1)
NOTE

#!/usr/bin/env bash
# Rootless FoxFang in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-foxfang-podman.sh launch        # Start gateway
#   ./scripts/run-foxfang-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   foxfang --container foxfang dashboard --no-open
#   foxfang --container foxfang channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      FOXFANG_GATEWAY_TOKEN|FOXFANG_PODMAN_CONTAINER|FOXFANG_PODMAN_IMAGE|FOXFANG_IMAGE|FOXFANG_PODMAN_PULL|FOXFANG_PODMAN_GATEWAY_HOST_PORT|FOXFANG_GATEWAY_PORT|FOXFANG_PODMAN_BRIDGE_HOST_PORT|FOXFANG_BRIDGE_PORT|FOXFANG_GATEWAY_BIND|FOXFANG_PODMAN_USERNS|FOXFANG_BIND_MOUNT_OPTIONS|FOXFANG_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-foxfang-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${FOXFANG_CONFIG_DIR:-$EFFECTIVE_HOME/.foxfang}"
ENV_FILE="${FOXFANG_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${FOXFANG_CONFIG_DIR:-$EFFECTIVE_HOME/.foxfang}"
ENV_FILE="${FOXFANG_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${FOXFANG_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${FOXFANG_PODMAN_CONTAINER:-foxfang}"
FOXFANG_IMAGE="${FOXFANG_PODMAN_IMAGE:-${FOXFANG_IMAGE:-foxfang:local}}"
PODMAN_PULL="${FOXFANG_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${FOXFANG_PODMAN_GATEWAY_HOST_PORT:-${FOXFANG_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${FOXFANG_PODMAN_BRIDGE_HOST_PORT:-${FOXFANG_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${FOXFANG_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$FOXFANG_IMAGE"
...
GATEWAY_BIND="${FOXFANG_GATEWAY_BIND:-lan}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'FOXFANG_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v foxfang >/dev/null 2>&1; then
    echo "Warning: foxfang not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    FOXFANG_CONTAINER="" FOXFANG_CONFIG_DIR="$config_dir" \
      foxfang config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    FOXFANG_CONTAINER="" FOXFANG_CONFIG_DIR="$config_dir" \
      foxfang config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  FOXFANG_CONTAINER="" FOXFANG_CONFIG_DIR="$config_dir" \
    foxfang config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${FOXFANG_GATEWAY_TOKEN:-}" ]]; then
  export FOXFANG_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "FOXFANG_GATEWAY_TOKEN" "$FOXFANG_GATEWAY_TOKEN"
  echo "Generated FOXFANG_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/foxfang.json"
...
PODMAN_USERNS="${FOXFANG_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported FOXFANG_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (FOXFANG_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${FOXFANG_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${FOXFANG_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$FOXFANG_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.foxfang/.npm \
    -e FOXFANG_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.foxfang:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.foxfang/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$FOXFANG_IMAGE" \
...
echo "Host CLI: foxfang --container $CONTAINER_NAME dashboard --no-open"
...
if [[ "$PLATFORM_NAME" == "Darwin" ]]; then
  ...
  echo "  FOXFANG_CONTAINER=$CONTAINER_NAME foxfang dashboard --no-open"
  echo "  One-time setup:"
  echo "    FOXFANG_CONTAINER=$CONTAINER_NAME foxfang config set gateway.controlUi.allowedOrigins '[\"http://127.0.0.1:18789\",\"http://localhost:18789\",\"http://127.0.0.1:28889\",\"http://localhost:28889\"]' --strict-json"

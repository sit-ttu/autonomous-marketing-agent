...
import { parseReleaseVersion } from "../foxfang-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const extensions = packageJson.foxfang?.extensions ?? [];

  if (!packageName.startsWith("@foxfang/")) {
    errors.push(`package name must start with "@foxfang/"; found "${packageName || "<missing>"}".`);
  }
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("foxfang.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("foxfang.extensions must contain only non-empty strings.");
  }
...
export function collectPublishablePluginPackages(
  ...
  for (const dir of dirs) {
    ...
    if (packageJson.foxfang?.release?.publishToNpm !== true) {
      continue;
    }
    ...
    publishable.push({
      ...
      installNpmSpec: packageJson.foxfang?.install?.npmSpec?.trim() || undefined,
    });
  }
...
export function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "foxfang-plugin-npm-view-"));

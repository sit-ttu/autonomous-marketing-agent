...
import {
  resolveBundledPackageManifest,
  resolveBundledPluginManifestPath,
} from "./bundled-plugin-source-utils.mjs";
...
function collectPluginSourceEntries(packageJson) {
  const packageManifest = resolveBundledPackageManifest(packageJson);
  let packageEntries = Array.isArray(packageManifest?.extensions)
    ? packageManifest.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageManifest?.setupEntry === "string" && packageManifest.setupEntry.trim().length > 0

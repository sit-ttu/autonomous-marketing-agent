...
export async function setupAuthTestEnv(
  ...
  process.env.FOXFANG_AGENT_DIR = agentDir;
...
export function requireFoxFangAgentDir(): string {
  const agentDir = process.env.FOXFANG_AGENT_DIR;
  if (!agentDir) {
    throw new Error("FOXFANG_AGENT_DIR not set");
  }
  return agentDir;
}

...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `foxfang-e2e-${name}-`));
  const configDir = path.join(homeDir, ".foxfang");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          FOXFANG_SKIP_PROVIDERS: "1",
          FOXFANG_SKIP_GMAIL_WATCHER: "1",
          FOXFANG_SKIP_CRON: "1",
          FOXFANG_SKIP_BROWSER_CONTROL_SERVER: "1",
          FOXFANG_SKIP_CANVAS_HOST: "1",
          FOXFANG_TEST_MINIMAL_GATEWAY: "1",
          VITEST: "1",
        },

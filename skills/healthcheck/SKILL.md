---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for FoxFang deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, FoxFang cron scheduling for periodic checks, or version status checks on a machine running FoxFang (laptop, workstation, Pi, VPS).
---

# FoxFang Host Hardening

## Overview

Assess and harden the host running FoxFang, then align it to a user-defined risk tolerance without breaking access. Use FoxFang security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim FoxFang changes the host firewall, SSH, or OS updates; it does not.
...
5. FoxFang gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if FoxFang can access sensitive data.
...
### 2) Run FoxFang security audits (read-only)

As part of the default read-only checks, run `foxfang security audit --deep`. Only offer alternatives if the user requests them:

1. `foxfang security audit` (faster, non-probing)
2. `foxfang security audit --json` (structured output)

Offer to apply FoxFang safe defaults (numbered):

1. `foxfang security audit --fix`

Be explicit that `--fix` only tightens FoxFang defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check FoxFang version/update status (read-only)

As part of the default read-only checks, run `foxfang update status`.
...
- Credential hygiene notes (location of FoxFang creds, prefer disk encryption)
...
After FoxFang install or first hardening pass, run at least one baseline audit and version check:

- `foxfang security audit`
- `foxfang security audit --deep`
- `foxfang update status`

Ongoing monitoring is recommended. Use the FoxFang cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `foxfang cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `foxfang update status`
...
Before creating, `foxfang cron list` and match on exact `name`. If found, `foxfang cron edit <id> ...`.
If not found, `foxfang cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `foxfang update status` (preferred for source checkouts and channels)
2. `npm view foxfang version` (published npm version)

## FoxFang command accuracy

Use only supported commands and flags:

- `foxfang security audit [--deep] [--fix] [--json]`
- `foxfang status` / `foxfang status --deep`
- `foxfang health --json`
- `foxfang update status`
- `foxfang cron add|list|runs|run`

Do not invent CLI flags or imply FoxFang enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by FoxFang compaction:

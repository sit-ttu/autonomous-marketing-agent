...
Goal: find the one real route from node -> gateway, verify FoxFang is advertising that route, then fix pairing/auth.
...
- whether `foxfang devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `foxfang qr --json`. It uses the same setup-code payload Android scans.

```bash
foxfang config get gateway.mode
foxfang config get gateway.bind
foxfang config get gateway.tailscale.mode
foxfang config get gateway.remote.url
foxfang config get gateway.auth.mode
foxfang config get gateway.auth.allowTailscale
foxfang config get plugins.entries.device-pair.config.publicUrl
foxfang qr --json
foxfang devices list
foxfang nodes status
```

If this FoxFang instance is pointed at a remote gateway, also run:

```bash
foxfang qr --remote --json
```
...
If `foxfang qr --json` says `Gateway is only bound to loopback`:
...
```bash
foxfang devices list
foxfang devices approve --latest
```
...
- `foxfang devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run foxfang qr again, rescan, then approve the pending device pairing.`

...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): FoxFangConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as FoxFangConfig;
  ...
  async function resolveReadToken(cfg: FoxFangConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: FoxFangConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();

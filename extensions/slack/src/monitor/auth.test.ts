...
vi.mock("foxfang/plugin-sdk/security-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("foxfang/plugin-sdk/security-runtime")>();
  return {
    ...actual,
...
describe("resolveSlackEffectiveAllowFrom", () => {
  const prevTtl = process.env.FOXFANG_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS;
  ...
  beforeEach(async () => {
    ...
    if (prevTtl === undefined) {
      delete process.env.FOXFANG_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS;
    } else {
      process.env.FOXFANG_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS = prevTtl;
    }
  });
  ...
  it("refreshes pairing-store allowFrom when cache TTL is zero", async () => {
    process.env.FOXFANG_SLACK_PAIRING_ALLOWFROM_CACHE_TTL_MS = "0";
    readStoreAllowFromForDmPolicyMock.mockResolvedValue(["u2"]);
    const ctx = makeSlackCtx(["u1"]);

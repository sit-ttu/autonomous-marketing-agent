...
const FOXFANG_OPENSHELL_E2E = process.env.FOXFANG_E2E_OPENSHELL === "1";
const FOXFANG_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const FOXFANG_OPENSHELL_COMMAND = process.env.FOXFANG_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "foxfang-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && FOXFANG_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: FOXFANG_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(FOXFANG_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-openshell-e2e-"));
      ...
      const gatewayName = `foxfang-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `foxfang-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/foxfang-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: FOXFANG_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("foxfang-openshell-e2e");

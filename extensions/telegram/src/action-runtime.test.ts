...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): FoxFangConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as FoxFangConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): FoxFangConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: FoxFangConfig, accountId = "media") {
    await handleTelegramAction(

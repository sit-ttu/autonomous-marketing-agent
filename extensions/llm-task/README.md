...
`foxfang.invoke --each`) without adding new FoxFang code per workflow.
...
This extension depends on FoxFang internal modules (the embedded agent runner).
It is intended to ship as a **bundled** FoxFang extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.foxfang/extensions` as a standalone plugin directory.

...
function getChannelConfig(cfg: FoxFangConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: FoxFangConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: FoxFangConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: FoxFangConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL FoxFang uses to send replies back to Chat.",
      ],

...
export async function handleNextcloudTalkInbound(params: {
  ...
  const defaultGroupPolicy = resolveDefaultGroupPolicy(config as FoxFangConfig);
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as FoxFangConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as FoxFangConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as FoxFangConfig);

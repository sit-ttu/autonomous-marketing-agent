...
import type { DiscordGuildEntry } from "foxfang/plugin-sdk/config-runtime";
import type { FoxFangConfig } from "foxfang/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "foxfang/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "foxfang/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "foxfang/plugin-sdk/setup-runtime";
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: FoxFangConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: FoxFangConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: FoxFangConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: FoxFangConfig; accountId: string }) =>

...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): FoxFangConfig {
    const defaults: NonNullable<NonNullable<FoxFangConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: FoxFangConfig) {

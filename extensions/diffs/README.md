...
Read-only diff viewer plugin for **FoxFang** agents.
...
Set plugin-wide defaults in `~/.foxfang/foxfang.json`:
...
Before:
FoxFang supports plugins.

After:
FoxFang supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/foxfang-diffs`).

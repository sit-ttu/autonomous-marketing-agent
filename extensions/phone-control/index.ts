...
function normalizeDenyList(cfg: FoxFangPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: FoxFangPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: FoxFangPluginApi) {
    ...
    const timerService: FoxFangPluginService = {

...
function resolveQianfanPreset(cfg: FoxFangConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: FoxFangConfig) => {
...
export function applyQianfanProviderConfig(cfg: FoxFangConfig): FoxFangConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: FoxFangConfig): FoxFangConfig {

...
vi.mock("foxfang/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("foxfang/plugin-sdk/media-runtime")>(
    "foxfang/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `foxfang-inbound-media-${crypto.randomUUID()}`);

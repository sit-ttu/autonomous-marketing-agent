...
import { VERSION } from "foxfang/plugin-sdk/cli-runtime";
import { danger, success } from "foxfang/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "foxfang/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "foxfang/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("foxfang channels login")}`,
              ),
            );
          }
        }

...
import { resolvePreferredFoxFangTmpDir } from "../../../src/infra/tmp-foxfang-dir.js";
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredFoxFangTmpDir(), "foxfang-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub FOXFANG_STATE_DIR.
    // Also keep it outside the FoxFang temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["FOXFANG_STATE_DIR"]);
    process.env.FOXFANG_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "foxfang-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredFoxFangTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default FoxFang state workspace and sandbox roots", async () => {
  ...
  it("rejects default FoxFang state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

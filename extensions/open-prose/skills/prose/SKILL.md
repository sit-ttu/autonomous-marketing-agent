...
metadata: { "foxfang": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == FoxFang `sessions_spawn`
- **File I/O** == FoxFang `read`/`write`
- **Remote fetch** == FoxFang `web_fetch` (or `exec` with curl when POST is required)

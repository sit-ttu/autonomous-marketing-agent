...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleFoxFangDevices.length > 0) {
      params.logger.warn(
        `matrix: stale FoxFang devices detected for ${params.auth.userId}: ${deviceHealth.staleFoxFangDevices.map((device) => device.deviceId).join(", ")}. Run 'foxfang matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {

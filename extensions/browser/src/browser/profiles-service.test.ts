...
import { resolveFoxFangUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveFoxFangUserDataDir: vi.fn(() => "/tmp/foxfang-test/foxfang/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "foxfang-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveFoxFangUserDataDir).mockReturnValue(userDataDir);

...
export function resolveFoxFangUserDataDir(profileName = DEFAULT_FOXFANG_BROWSER_PROFILE_NAME) {
...
export function buildFoxFangChromeLaunchArgs(params: {
...
export async function launchFoxFangChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveFoxFangUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_FOXFANG_BROWSER_COLOR).toUpperCase(),
  );

  // First launch to create preference files if missing, then decorate and relaunch.
  const spawnOnce = () => {
    const args = buildFoxFangChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateFoxFangProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
  ...
  try {
    ...
    log.warn(`foxfang browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopFoxFangChrome(

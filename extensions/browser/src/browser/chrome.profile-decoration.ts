...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".foxfang-profile-decorated");
}
 ...
 */
export function decorateFoxFangProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_FOXFANG_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_FOXFANG_BROWSER_COLOR).toUpperCase();

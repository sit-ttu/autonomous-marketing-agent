...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.FOXFANG_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "foxfang",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/foxfang" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    foxfang: { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchFoxFangChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "foxfang";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.FOXFANG_GATEWAY_PORT;
  process.env.FOXFANG_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.FOXFANG_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.FOXFANG_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.FOXFANG_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.FOXFANG_GATEWAY_PASSWORD;
  } else {
    process.env.FOXFANG_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}

  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          foxfang: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.FOXFANG_GATEWAY_PORT;
    process.env.FOXFANG_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.FOXFANG_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.FOXFANG_GATEWAY_PASSWORD;

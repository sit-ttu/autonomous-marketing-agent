...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => FoxFangConfig>(),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<FoxFangConfig["gateway"]>["auth"]> | undefined;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: FoxFangConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,

...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredFoxFangTmpDir: vi.fn(() => "/tmp/foxfang"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-foxfang-dir.js");
...
describe("pw-tools-core", () => {
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-foxfang-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredFoxFangTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredFoxFangTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredFoxFangTmpDir.mockReturnValue("/tmp/foxfang");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "foxfang-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".foxfang-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredFoxFangTmpDir.mockReturnValue("/tmp/foxfang-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "foxfang-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "foxfang-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredFoxFangTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "foxfang-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "foxfang-preferred", "downloads")}${path.sep}`),
    );
  });

...
function localFoxFangProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "foxfang",
...
function createLocalFoxFangResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localFoxFangProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveFoxFangUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "foxfang-reset-"));
    const profileDir = path.join(tempRoot, "foxfang");
    ...
    const ops = createLocalFoxFangResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveFoxFangUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "foxfang-reset-no-own-"));
    const profileDir = path.join(tempRoot, "foxfang");
    fs.mkdirSync(profileDir, { recursive: true });

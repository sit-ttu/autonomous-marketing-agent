...
import { loginOpenAICodexOAuth } from "foxfang/plugin-sdk/provider-auth-login";
import {
  DEFAULT_CONTEXT_TOKENS,
  normalizeModelCompat,
  normalizeProviderId,
  type ProviderPlugin,
} from "foxfang/plugin-sdk/provider-model-shared";
import { createOpenAIAttributionHeadersWrapper } from "foxfang/plugin-sdk/provider-stream";
import { fetchCodexUsage } from "foxfang/plugin-sdk/provider-usage";
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `foxfang models auth login --provider openai-codex` or `foxfang configure`.";
}

...
import type { FoxFangPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/foxfang-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): FoxFangPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("foxfang.webhook.received")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("foxfang.webhook.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("foxfang.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("foxfang.message.processed")?.add).toHaveBeenCalled();
    expect(telemetryState.histograms.get("foxfang.message.duration_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.histograms.get("foxfang.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("foxfang.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("foxfang.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("foxfang.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("foxfang.webhook.processed");
    expect(spanNames).toContain("foxfang.message.processed");
    expect(spanNames).toContain("foxfang.session.stuck");
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["foxfang.token"];
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("foxfang.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "foxfang.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["foxfang.reason"]).toBe("string");
    expect(String(attrs?.["foxfang.reason"])).not.toContain(
